...
///
/// Unprivileged and Tier 1: it reads repository-controlled text and computes
/// numbers and deterministic generated bytes. The caveats are printed on every
/// clean run, because a green accessibility check is exactly the output
/// someone would over-read; a failed run instead carries its concrete findings.
fn audit_tokens() -> Result<(), TaskError> {
    let tokens = partman_tokens::TokenSet::load_repository_tokens()
        .map_err(|error| TaskError::Policy(format!("could not read the design tokens: {error}")))?;
    let report = partman_tokens::audit(&tokens);

    println!("{}", report.summary());
    if report.is_clean() {
        let expected = partman_tokens::slint::render(&tokens).map_err(|error| {
            TaskError::Policy(format!(
                "could not generate the committed Slint token contract: {error}"
            ))
        })?;
        let generated_path = partman_tokens::slint::repository_generated_slint_path();
        let actual = fs::read(&generated_path).map_err(|source| TaskError::Io {
            path: generated_path.clone(),
            source,
        })?;
        if actual != expected.as_bytes() {
            return Err(TaskError::Policy(format!(
                "{} is stale; regenerate it with `cargo run -p partman-tokens --bin \
                 generate_slint_tokens -- --write`",
                generated_path.display()
            )));
        }
        println!(
            "generated Slint token contract is current: {}",
            generated_path.display()
        );
