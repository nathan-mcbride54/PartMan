...
| L8 | Designator stability | detach passthrough, reattach, recapture L2/L4/L7; one full VM reboot, recapture | client baseline | Which identity facts and designators survive replug and reboot unchanged on real hardware | fewer than two sittings; medium reprovisioned between sittings | `observed` — with L-F provisioned throughout, the array UUID, USB descriptor serial, and `by-id` set survived a passthrough replug and a full VM reboot unchanged; the `wwid` read failed `ENXIO` in both stability legs, as at every other capture — a failed read, not an observed absence (previously stated "`wwid` stayed absent"; restated by the 2026-08-13 readback's R2 below, whose scope names these legs); the replug's plug event visibly re-probed the udev db to the same single answer |
...
distinction is load-bearing. *(The L8 cell carried the same compression
as "`wwid` stayed absent" and was missed by this readback; it was
restated 2026-08-14, when the WP-L100 record sweep on issue #318 found
it. Both cells now read the same way.)* The reason it is load-bearing:
ADR-0019's naming rules currently define
...
### The USB-device-node recognition cell FR6 — preregistered 2026-08-19; taken the same day in both legs; established (one recorded instrument amendment on leg B)
...
| # | Cell | Command / API | Privilege | Distinguishing condition | Invalidation conditions | Result |
| --- | --- | --- | --- | --- | --- | --- |
| FR6 | What a client may read to recognize a USB device node on ADR-0034's ancestor walk | For the unit and for a non-USB contrast device: walk `/sys/class/block/<dev>/device/..`, `/../..`, … nearest first, exactly as the delivered predicate walks (parent components appended to the `device` link, no link resolution by the client beyond the platform's own), up to twelve ancestors; at each ancestor record the resolved path, the `subsystem` link's target, the `uevent` body, a bounded listing, and for each of a fixed attribute set (`idVendor`, `idProduct`, `serial`, `manufacturer`, `product`, `bDeviceClass`, `bInterfaceClass`, `busnum`, `devnum`, `devpath`, `vendor`, `device`, `class`, `modalias`, …) whether it exists, its mode, its read status and its bytes; and per ancestor two verdicts — the delivered predicate's (`idVendor` AND `idProduct` readable and non-empty) and the kernel's own (`uevent` `DEVTYPE=usb_device`); finally the nearest predicate-recognized ancestor's `serial`. Leg A as `nobody` on the node; leg B as `muser1` in the guest; double capture each | client baseline (the unprivileged actor on both legs); root only for the guest's read-only enforcement, the manifest, and the leg-A `runuser` | Whether the two classes separate: **every** ancestor the kernel classes `usb_device` (the device node, and the root hub above it) carries readable `idVendor` and `idProduct`, and **no** other ancestor (the SCSI target and host, the USB *interface* node, the PCI nodes) carries either — so the predicate's YES set equals the kernel's; which of the two reads, and which further attributes, distinguish the nearest device node from the root hub above it (the walk's nearest-first order is what keeps the serial the unit's and not the controller's); that the predicate's nearest-YES serial equals FR4's recorded `A20036CA8695D921` on both legs; byte-stable across the double capture; whether the real-hardware chain (leg A, with PCIe switches above the controller) and the passthrough chain (leg B) agree on all of the above | a USB interface node or a non-USB ancestor carrying readable `idVendor`+`idProduct` (the predicate would over-recognize — a finding that changes it); a `usb_device` node without both markers (under-recognition — likewise); the predicate's nearest-YES serial differing from FR4's; the unit absent from the guest (`void(apparatus)`); either leg's captures differing between themselves | **Established on both legs; the two classes separate, and the predicate's YES set equals the kernel's on every ancestor of both chains.** *Leg A* (the node, kernel `7.0.14-11-pve`, `nobody`, `CapEff` zero): the unit's walk is ten ancestors — `device/..` the SCSI target (`DEVTYPE=scsi_target`, none of the probed attributes), `../..` the SCSI host, `../../..` the **USB interface node** `2-3:1.0` (`subsystem` `usb`, `DEVTYPE=usb_interface`, `DRIVER=usb-storage`; carries `bInterfaceClass=08`, `bInterfaceSubClass=06`, `bInterfaceNumber`, `modalias` and **no `idVendor`/`idProduct`/`serial`**), `../../../..` the **USB device node** `2-3` (`DEVTYPE=usb_device`: `idVendor=0781`, `idProduct=55b1`, `serial=A20036CA8695D921`, `manufacturer=SanDisk`, `product`, `bDeviceClass=00`, `busnum=2`, `devnum=3`, `devpath=3`, `speed=5000`, `removable=removable`, all mode `444`), then the **root hub** `usb2` (also `DEVTYPE=usb_device`: `idVendor=1d6b`, `idProduct=0003`, `serial=0000:0a:00.0`, `product=xHCI Host Controller`, **`bDeviceClass=09`**, `devpath=0`, `speed=20000`), then four PCI nodes (the xHCI controller `0000:0a:00.0` and three PCIe ports; `vendor`/`device`/`class`/`subsystem_vendor` present, no USB attribute) to `pci0000:00`. The predicate answers YES at exactly the two `usb_device` nodes and no elsewhere; nearest-first lands on `2-3`, whose `serial` is FR4's recorded `A20036CA8695D921`. The NVMe contrast chain (four ancestors: the `nvme` class directory, the NVMe function `0000:01:00.0`, a PCIe port, `pci0000:00`) carries no USB attribute at any level. Two captures byte-identical. *Leg B* (VMID 9485, jammy 5.15.0-186, `muser1`, `CapEff` zero, the unit `blockdev --setro`): the same shape through QEMU's passthrough topology — target, host, interface `10-1:1.0` (same attribute set, no markers), device node `10-1` (`idVendor=0781`, `idProduct=55b1`, `serial=A20036CA8695D921`, `bDeviceClass=00`, `busnum=10`, `devnum=2`, `devpath=1`, `speed=5000`, `removable=unknown`), root hub `usb10` (`1d6b:0003`, `serial=0000:07:1b.0`, `bDeviceClass=09`, `devpath=0`), the emulated xHCI `0000:07:1b.0` and two PCI bridges; YES at exactly the two `usb_device` nodes; nearest-YES serial `A20036CA8695D921`; the virtio contrast chain (target, host, the `virtio` device with `vendor`/`device`/`modalias`, three PCI nodes) carries no USB attribute. All four stick captures (two per phase) byte-identical. **What distinguishes the unit's node from the root hub above it**, on both legs: `bDeviceClass` (`00` vs `09`, the hub class), `devpath` (`3`/`1` vs `0`), `idVendor` (`0781` vs `1d6b`), `product`; the walk's nearest-first order is what selects the unit, and the cell confirms it does. **One difference between the legs, recorded:** the device node's `removable` attribute reads `removable` on the node and `unknown` through the passthrough — a hypervisor-topology effect on an attribute the predicate does not read. **Instrument amendment (leg B):** phase 1's root script picked the contrast device as the first `lsblk` row whose transport was not `usb`, which on the guest was `loop0` (no `device` link), so the phase-1 contrast walk was empty; phase 2 on the same guest re-resolved the contrast to the virtio system disk and re-captured twice (the stick's captures stand and agree across all four); retained in the transcript |
...
**The sittings, 2026-08-19 (UTC).** *Leg A* (the Proxmox node `proxmox`,
PVE on kernel `7.0.14-11-pve`, udev 257): the unit at root port `2-3` as
`sdb`, `nvme0n1` the contrast; the instrument's digest recorded before the
first capture; run as `nobody` through `runuser` with `CapEff` all-zero at
both captures; the two captures identical but for the capture headers;
zero non-zero-rc lines. Transcript SHA-256
`0b1e08ff8eadd2f47c7d53693a8c4f1b694c1e1910ca8c53493b1417cd574ed0` (37497
bytes), computed on the node and on the workstation, agreeing. Taken before
the guest existed, so the unit was the node's. *Leg B* (VMID 9485): the
pinned jammy image verified against the pin before creation; the create
script read the unit's serial at host port `2-3` and refused to proceed on
any other; `usb0: host=2-3,usb3=1` and nothing else (asserted); `muser1`
through `runuser`, `CapEff` all-zero at all four captures; `blockdev
--getro` = 1 before capture; instrument digests recorded in-transcript
before the first capture and the amended phase's before its own; no
reboot; zero non-zero-rc lines. Teardown verified 2026-08-19T17:33:25Z;
the unit re-enumerated on the node afterwards. Transcript SHA-256
`aa2530c73878fe3e5fce1b460faab963fe3adee3f103395b5b0d3a1d2ed680b0` (63560
bytes), computed in the guest, on the host by the teardown script, and on
the workstation, all agreeing.

**Custody.** Both legs — transcripts and digests, the host environment
record, teardown proof, create/drive/teardown logs, and every instrument
including the amendment — archived at
`%USERPROFILE%\PartMan-evidence\2026-08-19-fr6-vmid9485\` on the operator
workstation, custodian Nate McBride.

**What this row now lets WP-L100 say.** The delivered predicate
(`USB_DEVICE_MARKERS`: `idVendor` and `idProduct` both readable) recognizes
exactly the set of ancestors the kernel itself classes `usb_device`, on a
real XHCI chain and through a passthrough chain, and recognizes no USB
interface node and no PCI or SCSI node; nearest-first lands on the unit's
node rather than the root hub's, and its `serial` is the unit's recorded
serial. The predicate's evidence moves from **none** to **FR6** in
`schemas/adapter-linux/fields.md` and in `naming.rs`'s own doc. What the
...
### The whole-device byte-window cell DR21 — preregistered 2026-08-19; taken the same day; established, valid on the second invocation with one recorded instrument amendment
...
| # | Cell | Command / API | Privilege | Distinguishing condition | Invalidation conditions | Result |
| --- | --- | --- | --- | --- | --- | --- |
| DR21 | Whole-device byte windows and geometry through a read-only handle, on real block devices and on loop devices carrying the catalogue images | Per disk, resolved by `/dev/disk/by-id` serial to its sysfs entry: the `dev` attribute against `stat -c %t:%T` of `/dev/<entry>` (the node the helper opens, identity-bracketed by device number); sysfs `size` and `queue/logical_block_size`/`queue/physical_block_size` against `blockdev --getsize64`/`--getss`/`--getpbsz` and the image length; SHA-256 of the first 65 536 bytes read through the node (`head -c`) against the image's first 65 536; SHA-256 of the last 65 536 bytes (`tail -c`) against the image's last 65 536; SHA-256 of the whole node against the image's; the same five comparisons on each image attached as a read-only loop device; once, `muser1` opening one node read-only (expected: denied) | root (the helper's context); the one denial line as the client | Whether the bytes a root process reads through a whole-device node, at its head and at its tail, are the medium's bytes — byte for byte, on a virtio-scsi disk and on a loop device — and whether the geometry the helper would take from sysfs (`size` × 512, `queue/logical_block_size`) is the geometry the block layer reports and the image has; whether the node the sysfs entry names is the device the entry describes | an imported volume reporting a size other than 4 194 304 bytes (the tail leg on that disk `void(apparatus)`; the loop leg stands); a by-id link missing (`void(apparatus)`); the two captures differing on any line; a short read | **Established.** On every one of the fourteen virtio-scsi fixture disks and every one of the fourteen read-only loop devices — twenty-eight nodes, two captures each: the sysfs `dev` attribute equals the node's `rdev` (`dev_agrees=yes` on all), sysfs `size` × 512 = `blockdev --getsize64` = 4 194 304 = the image length (`size_agrees=yes`), `logical_block_size`/`physical_block_size` = `--getss`/`--getpbsz` = 512 (`geometry_agrees=yes`), and the SHA-256 of the first 65 536 bytes, the last 65 536 bytes and the whole node each equal the image's (`head_agrees`/`tail_agrees`/`whole_agrees` all `yes`; **zero disagreeing lines in either capture**). `muser1` opening one node read-only: `Permission denied`, rc 1 (mode `660 root:disk`, the client in no group). The bytes a root process reads through a whole-device node are the medium's, at the geometry sysfs states, on both leg shapes |
...
**The sittings, 2026-08-19 (UTC), one guest (VMID 9492), two invocations.**
The base image and all fourteen fixture images verified against their pins
before creation (create log in the bundle); the images re-verified in the
guest against `MANIFEST` before any capture (`14/14`, the manifest token
in-transcript); instrument digests recorded in-transcript before the first
capture; root the measuring actor (`euid: 0`, `CapEff` full — the helper's
own context, the inverse of the client sittings' all-zero gate); nothing
installed; no reboot. **Invocation 1 is void(instrument) and retained**: every
measurement line agreed (`NO_count` 0 in both captures) but the comparison
included the instrument's own numbered `capture N` header lines, so two
byte-identical measurement sets compared unequal on labels the instrument
itself prints — transcript SHA-256
`5e54da70c6e06134bcf7918a8ac36353a92b435dd1e2ae5cac716c4b7e1ce928`, its key
lines archived (the raw bundle was not retrieved before teardown — an
archival gap recorded here, costing nothing the cited run does not carry).
The amendment: the capture body carries no capture number; the driver names
the capture outside the compared files. **Invocation 2, the cited run**: the
two captures byte-identical (`captures_identical=yes`), zero disagreeing
lines, zero void lines, the client denial verbatim. Transcript SHA-256
`0d51270d7ceed781ea3dcb405674bae9f83c763de074d0fc651bd338d318d61c` (62 962
bytes), computed in the guest and on the workstation, agreeing. Teardown
verified 2026-08-19T22:28:57Z; no volume and no LVM remnant.

**Custody.** The bundle — the cited transcript and its digest, both capture
files, the host environment record with the fourteen image digests, the
teardown proof, and the void invocation's key lines — archived at
`%USERPROFILE%\PartMan-evidence\2026-08-19-dr7-bytewindow-vmid9492\` on the
operator workstation, custodian Nate McBride.

**What this row now lets the helper say.** HLP-002's byte layer reads
through exactly this shape — a read-only handle on the whole-device node,
bracketed by device number, windowed at head and tail under sysfs geometry —
and this row establishes the host half on real block devices and on loop
devices: the windows are the medium's bytes, byte for byte, and the
geometry is the block layer's. The helper's reach declaration cites this
...
### The apply-ceremony cells DR22–DR24 — preregistered 2026-08-19; taken the same day on all three tiers; established, with two recorded instrument scope notes
...
| # | Cell | Command / API | Privilege | Distinguishing condition | Invalidation conditions | Result |
| --- | --- | --- | --- | --- | --- | --- |
| DR22 | The ceremony's tooling, per tier | Root, package manager and filesystem: `pkcheck`, `pkttyagent`, `polkit-agent-helper-1`, `busctl`, `dbus-send`, `gdbus` — installed by default? which package? version banner (bounded, one launch each)? mode and setuid bit? Which policy dialect directory exists (`/etc/polkit-1/localauthority` for the 0.105 line, `/etc/polkit-1/rules.d` for the 12x line)? Is the D-Bus authority service file present and is `polkitd` running before anything asks? | root | Whether the two candidate mechanisms for "the helper asks polkit about its client" — a `pkcheck` invocation and a D-Bus call — have any implementation present on a default image, and whether a text agent exists to answer a challenge | a tier image failing to boot with cloud-init (`void(apparatus)`); a version probe hanging past its 5 s bound (a finding, capture still valid); the two captures differing | **Established; the three tiers differ exactly where the route cares.** *jammy* (polkit 0.105): `policykit-1`/`polkitd`/`pkexec` 0.105-33ubuntu0.1, `pkexec` setuid `4755`; **`pkcheck` 0.105 and `pkttyagent` 0.105 both present, both owned by the `polkitd` package**; `busctl`, `dbus-send`, `gdbus` present; the **0.105 dialect** — `/etc/polkit-1/localauthority` (`700`) and `/var/lib/polkit-1/localauthority`, and **no** `/etc/polkit-1/rules.d`; the D-Bus activation file present; **`polkitd` already running before anything asked** (`polkit.service` loaded/active/static). *Debian 12* (polkit 122): `polkitd` 122-3 with `libpolkit-gobject-1-0`/`libpolkit-agent-1-0`; **`pkexec` absent** (bookworm splits it out) but **`pkcheck` 122 and `pkttyagent` 122 present, both from `polkitd`**; `busctl`/`dbus-send` present, **`gdbus` absent**; the **12x dialect** — `/etc/polkit-1/rules.d` and `/usr/share/polkit-1/rules.d` both `700` owned by `polkitd`, no `localauthority` tree; the activation file present; **`polkitd` not running before anything asked** (`polkit.service` inactive). *Arch* (the 2026-08-15 cloud image, systemd 261, `dbus-broker` 37): **nothing** — no `pkcheck`, no `pkaction`, no `pkttyagent`, no agent helper, no activation file, `polkit.service` `not-found`; only `busctl`/`dbus-send`/`gdbus` and the empty `/usr/share/polkit-1/rules.d` scaffolding. *On both polkit tiers* the setuid agent helper exists under both `/usr/lib/polkit-1/` and `/usr/libexec/`, one a link to the other, and the pair's modes are mirror images between the tiers (jammy `777`/`4755`, Debian `4755`/`777`) — recorded verbatim because a package that installs an agent must not assume one path |
| DR23 | **The authorization matrix**: what polkit answers a root daemon asking about another process | With the test policy installed: as **root**, `pkcheck --action-id <id> --process <pid>` for each subject (S1 session-less `sleep`, S2 `ssh`-session `sleep`, root's own shell), for each action (`auth_admin`, `yes`), with and without `--allow-user-interaction`, each bounded to 10 s — recording **exit status and stderr verbatim**; beside each, `loginctl show-session`/`loginctl show-user` for that subject (session id, class, `Active`, `Remote`, seat) and `ps -o pid,lstart` so the subject is identified the way polkit identifies it | root measures; the client only holds the subject processes | (1) Whether `pkcheck` can answer *about another process* at all; (2) what an `auth_admin` action returns for a session-less subject, for an `ssh`-session subject, and for root itself; (3) whether `--allow-user-interaction` with no agent returns a refusal or a hang; (4) **whether an `ssh` login session is `Active` to logind** — the fact that decides whether `allow_active` can ever cover a headless caller, or whether the action must declare `allow_inactive`/`allow_any` | a subject process exiting before it is checked (that arm `void(apparatus)`, re-run); `pkcheck` hanging past its bound (recorded as the finding it is, capture still valid); the two captures differing on any line | **Established on both polkit tiers, byte-identical across both captures and identical between the tiers.** (1) **A root caller can ask polkit about another process**: `pkcheck --action-id … --process <pid>` resolved every client subject and returned **rc 0 for the `yes` control** on each — the subject is evaluated, not rejected, which is the mechanism the ceremony needs. (2) **`auth_admin` for any client subject refuses, bounded**: rc 2, `Authorization requires authentication and -u wasn't passed.`; with `--allow-user-interaction`, rc 2, `Authorization requires authentication but no agent is available.` — never a hang, never a grant. (3) **The root subject is authorized for `auth_admin` with no agent and no prompt**: rc 0 on both tiers, with and without interaction — root is already an administrator to polkit, so a check whose *subject* is the helper itself would authorize nothing meaningful. (4) **The session fact the headless question turns on**: every login session on these images — root's `ssh` session and `muser1`'s — reads `Class=user Active=yes State=active Remote=yes Seat=` (no seat). **An `ssh` login session is `Active` to logind on both polkit tiers**, so an `allow_active` action does cover a remote CLI user; what DR20 and increment 1's H2a measured was a `runuser` client with **no session at all**, and the generalisation drawn there (“a CLI user over `ssh` … is not ‘active’ to polkit either”) is measured **false** and corrected on `docs/work-packages/WP-L110.md` |
| DR24 | The text agent and the D-Bus authority | As the client (S2, with a session): `pkttyagent --process <pid>` bounded to 10 s, registered in the background, then root re-runs the DR23 `auth_admin` check **with** `--allow-user-interaction` — recording whether a challenge is offered (and to which identity), verbatim, with no password typed; as the client (S1, no session) the same, expecting a registration refusal recorded verbatim. Separately, as root with no session: `busctl call org.freedesktop.PolicyKit1 /org/freedesktop/PolicyKit1/Authority org.freedesktop.PolicyKit1.Authority EnumerateActions` (bounded) and an introspection of `CheckAuthorization`'s signature — does D-Bus activation of `polkitd` work from a root caller with no session (the launch round's second unmeasured item), and what subject shape does the method require? | root and client | Whether a challenge can be delivered to a headless-but-sessioned caller through a text agent; whether it can be delivered to a session-less one; and whether the D-Bus route is reachable from the helper's own context without a session — the three facts the ceremony route is chosen on | `pkttyagent` absent on a tier (that tier's arm is *absent*, a result, not a void); a bounded call hanging (recorded); the two captures differing | **Established.** (1) **A text agent cannot be started for a client by a context with no controlling terminal**: `pkttyagent --process <pid>` returned rc 127, `Error creating textual authentication agent: Error opening current controlling terminal for the process (`/dev/tty'): No such device or address (polkit-error-quark, 0)` — identically on both tiers and for both the session-less and the `ssh`-session subject, because the instrument starts it from a script with no tty. **Scope, stated rather than glossed:** this establishes that *no daemon and no script can register an agent on a client's behalf*; it does **not** test `pkttyagent` in a real interactive terminal, which this sitting did not have. (2) With no agent registered, root's interactive re-ask returns the same bounded rc 2 `no agent is available` — the fail-closed answer is deterministic. (3) **The D-Bus authority answers a root caller**: `busctl call … EnumerateActions` rc 0 (117 actions on jammy, 86 on Debian 12), and **on Debian 12 `polkitd` was inactive before the call and running after** — D-Bus activation on demand from a root caller, the launch round's second unmeasured item, established on the tier where it matters. `CheckAuthorization`'s signature, verbatim: `(sa{sv})sa{ss}us` → `(bba{ss})`, beside `RegisterAuthenticationAgent (sa{sv})ss`. (4) *Arch*: `Call failed: The name is not activatable`, `polkitd` not running after — there is nothing to ask on a default image |
...
**The sittings, 2026-08-19 (UTC), three guests in parallel** (jammy VMID
9494, Debian 12 VMID 9495, Arch VMID 9496, all destroyed). Each image
verified against its pin before creation; instrument digests recorded
in-transcript before the first capture; `muser1` in three subject classes;
root the setup and measuring actor; nothing installed; no reboot; the test
policy installed with digest
`9f1ca64ecd1fde2304a34437a101b872972e372e885b1d9a3773e3b905e18f85` on both
polkit tiers and **removed before teardown on every tier** (verified
in-transcript); no password typed and none recorded. The two captures of
the DR23 matrix are **identical on both polkit tiers**, and the tiers agree
with each other line for line. Teardowns verified 23:53:31Z (9495),
23:53:36Z (9494), 23:53:40Z (9496); no volume and no LVM remnant on any.
Transcript SHA-256s, each computed in the guest and again on the
workstation, agreeing: jammy
`7322e50e7728d991c89b6f83a12b85a288b2622dff52228c64745a67a55084b2` (9 747
bytes); Debian 12
`8b71f66d9dcd23a4f30a85412bfec2e99db4a7000db4cb57c896646e0b96b5f7` (9 776
bytes); Arch
`8f2cddd40500fbc460c51ea9189760a0d1daa70482311dd63bec8b024bbf22e9` (4 996
bytes).

**Two instrument scope notes, recorded rather than discovered later.**
(1) The subject facts print a `loginctl` session looked up **by uid**, so
all three `muser1` subjects show that user's `ssh` session (17); the
per-process truth is in the same lines' `/proc/<pid>/sessionid` — 16 for
the `runuser` and `su -` subjects, inherited from root's own session, and
17 for the `ssh` subject. Every `pkcheck` answer is taken per pid and is
unaffected; what the coarse lookup cannot do is attribute a session
*class* to the first two subjects separately, so the cell's session claim
is made about the `ssh` session it names and about no other. (2) The
`pkttyagent` arm ran through `runuser` from a script, so it measures the
no-terminal case, as its result says. Beyond those: root held a logind
session in every guest (the drive is itself an `ssh` login), so **a root
caller with *no* session was not isolated** — what is established is a
root caller *with* one; that residue is named here rather than left for a
reader to assume away.

**Custody.** The three bundles — transcripts and digests, the host
environment records, the per-tier driver logs, the teardown proofs, and
the three instruments (`dr8-root.sh`, `drive-dr8.sh`, `sit-dr8.sh`) —
archived at
`%USERPROFILE%\\PartMan-evidence\\2026-08-19-dr8-ceremony-vmid9494-9496\\`
on the operator workstation, custodian Nate McBride.

**What these rows now let WP-L110's ceremony round say.** The helper *can*
ask polkit about its client, on both tiers that have polkit, by two
present-and-answering mechanisms: a `pkcheck` invocation (a binary, so
SAFE-004's launcher, whose home is an undecided route) or the D-Bus
authority (a client library, so a supply-chain edge) — and the round
now chooses between measured options rather than believed ones. An
`auth_admin` action never grants a client without an agent, and refuses
**bounded and typed**, so a fail-closed helper is straightforward. The
agent must be started **by the client, in its own terminal** — no
daemon can register one on a client's behalf — which fixes the shape of
the ceremony: the helper asks, the client's own agent answers, and a
client with no agent receives a refusal the helper can report with a
remediation naming `pkttyagent`. A remote `ssh` CLI user **is** `Active`,
...
### The mutation-tool-presence cell DR25 — preregistered 2026-08-20; taken the same day on all three tiers; established, valid on the second invocation with one recorded instrument amendment
...
| # | Cell | Command / API | Privilege | Distinguishing condition | Invalidation conditions | Result |
| --- | --- | --- | --- | --- | --- | --- |
| DR25 | The candidate mutation tools on each tier's default image | Root, filesystem and package manager, double capture, per tier: **by name** — `sgdisk` and `sfdisk` (path via `command -v` plus the fixed `sbin` locations, mode and owner); the **`mkfs.*` family enumerated by glob** over the PATH's `bin`/`sbin` directories, so absence is measured rather than assumed, read against LIN-002's list — ext2/3/4, Btrfs, XFS, F2FS, FAT/exFAT, NTFS — plus `mkswap` for LIN-002's swap; `libblockdev` by `ldconfig -p` entries matching `libblockdev`/`libbd_` and the shared objects' paths. **By the package manager's view** — the owner of each present binary (`dpkg -S` / `pacman -Qo`) and its installed version (`dpkg-query -W` / `pacman -Q`); the same queries for the absent candidates by their tier package names (`gdisk`, `util-linux`, `e2fsprogs`, `btrfs-progs`, `xfsprogs`, `f2fs-tools`, `dosfstools`, `exfatprogs`, `ntfs-3g`, and the tier's `libblockdev` packages), exit status recorded — "not installed" is a measured answer, never an inference. **One bounded (5 s) version invocation per present binary** — `--version`, falling back to `-V` — exit status and first output line verbatim | root | Which of route (b)'s two unmeasured options — authoritative native tools; libblockdev — have any implementation present on a default tier image, and at what version: whether a default image can write a partition table at all without a package install, and which of LIN-002's file-system makers exist by default — each absence a CAP-004 tool-floor fact the toolset round must price and a packaging fact for LIN-008, per tier | a tier image failing to boot with cloud-init (`void(apparatus)`); a version probe failing or hanging past its 5 s bound (a finding, recorded verbatim, the capture still valid); the two captures differing; any package-list mutation (`void(instrument)` for that tier — nothing is installed) | **Established; the tiers differ exactly where the route cares.** (1) **Both table writers are present by default on every tier**: `sgdisk` — jammy 1.0.8 (`gdisk` 1.0.8-4build1), Debian 12 1.0.9 (`gdisk` 1.0.9-2.1), Arch 1.0.10 (`gptfdisk` 1.0.10-2); `sfdisk` and `mkswap` — util-linux 2.37.2 / 2.38.1 / 2.42.2, **three distinct feature generations of the same tool across the tiers**. (2) **The mkfs family is tier-gapped**: ext2/3/4 present everywhere (`mke2fs` via `e2fsprogs` 1.46.5 / 1.47.0 / 1.47.4); Btrfs on jammy (`btrfs-progs` 5.16.2) and Arch (7.1) and **absent on Debian 12**; XFS on jammy only (`xfsprogs` 5.13.0); FAT on jammy and Arch (`dosfstools` 4.2) and **absent on Debian 12**; NTFS on jammy only (`ntfs-3g` 1:2021.8.22); **F2FS and exFAT absent on every tier**; the glob sweep found no maker for them under any name. (3) **libblockdev is present on jammy only** — 2.26, eight plugins including `libbd_part.so.2` and `libbd_fs.so.2`, the `libblockdev*2` package family that rides in with the image's `udisks2` (which every WP-020 acceptance guest purges as a recorded deviation) — and **absent by the loader and by name on Debian 12 and Arch**. (4) **A launched version query does not cover the family**: `mkfs.fat`/`mkfs.vfat` refuse both `--version` and `-V` (rc 1, dosfstools 4.2, on both tiers that ship them); `mkfs.ntfs --version` exits 0 with an empty first output line; the `mke2fs` family answers only `-V` (rc 1 on `--version`); `sgdisk`, `sfdisk`, `mkswap`, `mkfs.btrfs` and `mkfs.xfs` answer `--version` rc 0. The package manager's record carries every version the launch could not. (5) The package-database digest is identical before and after on every tier — nothing installed. One deterministic instrument limitation, stated: on the two usr-merged dpkg tiers, `dpkg -S` of a resolved `/usr/sbin` path answers nothing (the database records the `/sbin` alias), so the per-binary owner lines read `unowned` there; ownership and version come from the explicit package queries beside them, and Arch's `pacman -Qo` resolves every binary |
...
**The sittings, 2026-08-20 (UTC), three guests in parallel, twice.**
**Invocation 1 (jammy VMID 9499, Debian 12 VMID 9500, Arch VMID 9501,
all destroyed): void(instrument), retained.** Its version probe ran
`timeout 5 <tool> --version 2>&1 | head -1` and took the pipeline's exit
status: `head` exits after one line, so whether the tool finished its
write before `SIGPIPE` reached it is a race, and the recorded rc flapped
(0/141/1) over byte-identical output — the jammy leg's two captures
differed on exactly three rc values and its double-capture gate said
`NO`, refusing precisely the nondeterminism it exists to refuse. The
Debian 12 and Arch legs passed byte-identical, but their recorded rcs
carry the same taint, so the whole invocation is superseded rather than
patched tier by tier. The corrected instrument captures the tool's full
output first and takes the first line afterwards — no pipe, no race.
**Invocation 2, the cited run (jammy VMID 9502, Debian 12 VMID 9503,
Arch VMID 9504, all destroyed): valid on every tier.** Each image
verified against its pin before creation; instrument digest
`80876d07745a149264fdebce0aafbf48220132fd89968d1a2a62fd003699d761`
(`dr9-root.sh`) recorded in-transcript before the first capture; root
the measuring actor (`CapEff` all-ones, the helper's context); guest
kernels 5.15.0-186-generic / 6.1.0-52-cloud-amd64 / 7.1.8-arch1-3; both
captures byte-identical and the package database unchanged on every
tier. Teardowns verified 14:25:38Z (9502), 14:25:22Z (9503), 14:25:38Z
(9504); no volume and no LVM remnant on any. Transcript SHA-256s, each
computed in the guest, on the host after transfer, and on the
workstation after archiving, all agreeing: jammy
`ca039aa48100f984bc9678f373e8ed4adade6b39ab6c82b8f1e49e8e3ed4e47c`
(11 022 bytes); Debian 12
`712e78536341bf98fc479b2616ffa33157de961fc9cf0e2a08ff8a3f68ef7a80`
(6 773 bytes); Arch
`2580a3cc00ccf0114fe4a09f886e912839e7fa469d752adaade330525b4eedae`
(8 807 bytes).

**Custody.** The six bundles — the cited run's three (transcripts,
digests, driver logs, teardown proofs) and invocation 1's three retained
beside them — with the three instruments (`dr9-root.sh`, `drive-dr9.sh`,
`sit-dr9.sh`), archived at
`%USERPROFILE%\PartMan-evidence\2026-08-20-dr9-toolpresence-vmid9499-9504\`
on the operator workstation, custodian Nate McBride.

**What this row now lets WP-L110's toolset round say.** The
authoritative-native-tools option has universal substrate for the table
half — `sgdisk` and `sfdisk` on every default tier image, at three
different version generations — and tier-gapped substrate for the mkfs
half, with two of LIN-002's file systems (F2FS, exFAT) having no maker
on any default image. The libblockdev option has one default tier of
three. And CAP-004's "verified version" cannot rest on launching the
tool for the whole family — dosfstools answers no version spelling at
all — so a version verification discipline needs the package manager's
record or a content identity beside the launch.
