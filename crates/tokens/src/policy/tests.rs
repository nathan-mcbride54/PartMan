...
use crate::policy::{
    ...
    carries_meaning, required_meaning_bearing_roles, threshold_for,
};
...
fn the_vocabulary_matches_the_specification_it_claims_to_come_from() {
    ...
    // ordinal severities; UI-011 names eight progress states. Counting them
    // here means adding or dropping one is a deliberate edit to a test that
    // says which requirement it serves.
    assert_eq!(REQUIRED_ENTITY_ROLES.len(), 8, "UI-003 entity types");
    assert_eq!(
        REQUIRED_SEVERITY_ROLES.len(),
        5,
        "PLAN-004 severities 0..=4"
    );
    assert_eq!(REQUIRED_PROGRESS_ROLES.len(), 8, "UI-011 progress states");
    ...
    for severity in [
        ...
        assert!(
            REQUIRED_SEVERITY_ROLES.contains(&severity),
            "{severity} is one of PLAN-004's five classes"
        );
    }
}
