...
| Version | Summary |
| --- | --- |
| 19.0.0 | Resolved SI-41 (ADR-0055, on the Linux transport route round of 2026-08-19): **RPC-001's Linux clause is revised, and SAFE-002 is untouched.** The clause read "Unix domain socket (0700, root-owned directory) with peer-credential verification"; a Unix socket is connected to by path, the kernel requires the connecting process to hold search permission on the directory and write permission on the socket inode, and a `0700` root-owned directory admits root alone — measured (a child dropped to uid 65534 with supplementary groups cleared gets `EACCES` before the listener sees anything; a `0711` root-owned directory admits it and `SO_PEERCRED` reports its credentials) — so the clause excluded the SAFE-002 client it exists to serve, leaving HLP-003 and HLP-007 with no caller to identify. The clause now reads "Unix domain socket in a root-owned directory searchable but not writable by others (mode 0711), the socket node owned by the authorizing user and accessible to that user alone (mode 0600), with peer-credential verification of the connecting process against that user": two kernel-enforced gates before the helper parses a byte — the Linux analog of the Windows SDDL clause — and the credential check verifies what the kernel admitted. The Windows and macOS clauses stand verbatim. The credential route is `std` plus `rustix`'s safe `socket_peercred` (no first-party `unsafe`, a Linux-only dependency the workspace already carries), flat per-user nodes under one directory, per-message credentials deferred, the transport in its own crate under WP-040. Major: a MUST's sentence changes meaning. |
| 18.0.0 | LIN-001's discovery route is decided, and the UDisks2 floor moves to the tool it gates (ADR-0054, on the Linux UDisks2 route round of 2026-08-19 and the DR16–DR19 rows). **Two sentences change meaning, so this is major.** LIN-001 read "Use UDisks2 for discovery/authorization and libblockdev or authoritative native tools for mutations"; the delivered Linux discovery layer reads the kernel's and the distribution's client-readable interfaces — sysfs, the udev database, procfs, `os-release` — each entered by a measured row (nineteen rows on three tiers), opens no device node and launches no process, and the assignment had claimed LIN-001 increment-gated rather than let that route become LIN-001 by drift. The round costed the literal reading (a D-Bus client as the client's discovery interface: zero rows against nineteen, a provoked `udisksd` launch SAFE-004 cannot see, second-hand provenance over the same udev database, and a read-only product `blocked` on every default Debian 12 and Arch install) and decided for the measured route; LIN-001 now says so, and reserves UDisks2, libblockdev or authoritative native tools for authorization and mutations behind the helper's own recorded route decision (WP-L110; HLP-002, LIN-009). Section 9's Debian/Ubuntu row loses its third conjunct: "UDisks2 ≥ 2.9" was a per-tool floor sitting in a platform row, and Section 9's own closing sentence says per-tool floors live in the CAP-006 store; it moves there, entered by the first package that invokes UDisks2 with ADR-0054 as its basis, gating the operations that use the tool through CAP-004's delivered machinery. The floor's remaining conjuncts are measured on every tier (DR16–DR19); the engine's never-widen rule is untouched — the row changed by ADR, as Section 9 permits. No §1.11 item: no requirement-versus-requirement pair conflicts; the tension was requirement-versus-delivery, which is what the round and this ADR are for. |
| 17.4.0 | The Linux host-assembled naming designations (ADR-0053, on the Linux host-assembled designation round and the DR1–DR14 rows): **ADR-0034's revisit condition fired in its sanctioned direction, and the table extension lands with its rows.** Three cells gain designations, each a direct sysfs source measured for value, stability across re-assembly and a reboot, and per-unit distinctness: the (Linux, Aggregate, mdraid) designator is **`md/uuid`** under the array's block-class node, bytes verbatim, trailing newline included — the udev cache's `MD_UUID` spells the same bits differently and is rejected as the second, cached source; the (Linux, Volume, LVM2 logical volume) name is **`dm/name`**, verbatim, its role undesignated; the (Linux, `BackingExtent`, loop) path is **`loop/backing_file`**, verbatim, its host the file-system node. Two cells stay undesignated on measured grounds: the LVM2 volume-group id (no client-readable interface reports it — L7 — so every volume group is a designator-absent aggregate and its logical volumes indeterminate non-operands until the helper names it) and the dm-crypt mapping name (DR12 measured it as the opener's argument, not a stored property). The kind is decided by DR3's markers and `dm/uuid`'s prefix as a classification input, never by entry name, which DR2 measured renumbering across one reboot. Rejected and recorded: `MD_UUID`, `dm/uuid` as designator or name, member `ID_FS_UUID`, the mapper name for dm-crypt, entry names, and holding for the member-signature question, which is a different question (DR14: no interface reports a signature offset) and gets its own round. **Minor under §0.1**: previously undesignated cells gain designations; LIN-006, INV-004 and Section 5 stand verbatim; no text narrows. |
| 17.3.0 | A relocation consumes its own destination (ADR-0052, on issue gitea#371's destination-vocabulary round). PART-005 requires moves, overlapping ones by journaled chunk copy, and the model carried no destination vocabulary — `Move | Copy` refuse as `NotRepresentable`, pinned by `no_representable_request_relocates_bytes`. The round's revision 1 was broken by an adversarial pass and rewritten around a **trilemma**: for an overlapping move with source S and destination D, closure reach needs every rewritten byte in `destroyed ∪ consumed ∪ written`; the delivered `resolve_step_output` contract needs `consumed` to be exactly one range equal to the moved partition's post-move extent, `[D]`; and ADR-0018's consumed class forbids a consumed range intersecting any existing node's extent, which `[D]` does when S∩D ≠ ∅. **The consumed class gives**: ADR-0018:135-137 is amended so a relocation's consumed range may intersect the extent of the step's own target and of nodes named within it, and no other's — `destroyed = S`, `consumed = D`, `written = T`. The precise and hybrid declarations were rejected because the reversal draft cannot spell its target under either and `emit_reversal` would refuse the whole forward plan; widening `resolve_step_output` instead was rejected as changing a WP-010 contract with a golden vector for a case with one lawful, already-expressible answer. Decided with it: `Move { target, new_start }` as an authored byte offset; the destination rule `D ⊆ free_extents(host) ∪ S`, aligned, inside the host, outside the scheme regions, and not intersecting any node that is neither the target nor named within it (the literal "no other node" form was measured to refuse every downward overlapping move of an ordinary partition with a device-framed file system); the copy mode derived from S∩D; a `MoveDraft` reversal with an unoccupied-source precondition; and the hosted-signature enumeration discharged **by naming position, not by family** — a node named within the moved target moves with it and is preserved; one not named within it is refused by the destination rule inside S∩D and, inside S\D, released with the source range and destroyed, the authenticated closure saying so — with the `Consequence` variant enumerating that release and its negative space bounded explicitly (kind-level; no partition type or role; its silence is not a boot verdict) and the "checking boot consequences" justification dropped. **Minor under §0.1**: PART-005 and §11.2's node-overlap sentence stand verbatim; the changed sentence is an ADR's, and the major counter-argument (the consumed class as load-bearing for §11.2) is recorded and declined — the constructor enforces no such check today, the invariant is over nodes not step ranges, and the exception admits only bytes no other node can hold. Named rather than discovered: a post-move target spelling for later steps is a WP-050 obligation; §11.2's interrupted-move invariant is an apply-time WP-070 dependency, deferred; the tripwire comes down in the same change as its producer with a named replacement PART-005 traceability row; the stale `StepRanges::consumed` doc-comment is WP-010's to correct. No Rust lands with the ADR; the WP-060 increment owes its sitting. Issue #370 is not moved. |
| 17.2.0 | Descent admits an unlocated child of a geometric parent (ADR-0051, resolving issue #319's authorization half — the oldest live hole in the tree, on which three predicates had died). `descends_into`'s last arm read `kind != EdgeKind::Containment`, so a source declaring bytes never descended into a containment child declaring none. Measured on the committed whole-disk ZFS vdev: removing the signature's **one extent fact**, which nothing requires, took every mutating operation on the disk from **refusing 10 of 10 to `Clear` 10 of 10** over a live pool, because the signature is reached only by the byte scan and the descent that would find it was refused. Honest absence failed **open** at the one arm that needs it. **The round's finding is that the arm's own comment described a job it does not do**: it justified itself as preventing capture of "a sibling that merely lacks a fact", and admitting wholesale does red the sibling pin — but the captured node's parent is a *partition table*, whose extent is its own header bytes rather than the region it governs, so descent from a table into any partition must be refused and the geometric comparison already refuses it when the child declares an extent. The carve-out was extending that refusal to extentless children **by accident**, under a name describing something else, and the predicate that actually decides it — ADR-0041's `containment_pair_is_geometric` — had **never been consulted by this arm**. Descent now admits an unlocated child where the pair is geometric and refuses it where the pair is structural. **One red**, the pinned open limit ADR-0048 committed for this very issue, closing deliberately; both sibling pins survive, which is what distinguishes this from the wholesale form. Two mutations, each proven applied, each killed — the second by the act's own new structural regression alongside both pre-existing pins. **Minor under §0.1**, and this corrects the framing recorded on the issue: for a geometric pair this section's sentence was right and the code deviated, but for a structural pair the refusal is real and the sentence did not carry it, so the sentence gains a precision it never stated — an addition, not an unversioned conformance fix. ADR-0018's theorem is undisturbed: the premise is over the edge taxonomy, which is untouched, and the act only ever adds reach. **Not claimed closed**: issue #319's third measured shape did not reproduce on the committed fixture this round and is recorded as unmeasured rather than discharged. |
| 17.1.0 | A backing extent is framed on its named host (ADR-0050, resolving issue #365). ADR-0046 enforced the anchoring rule for every kind but one: a backing extent lies outside every containment forest, so `frame_root` is `None` for it and the frame check never ran, and no edge kind may target one, so the edge-versus-extent cross-check never saw it either. **It was the single node in the model whose declared frame nothing constrained**, and three acts had to pin limits around that hole — ADR-0047's name-unrecoverable aggregate, ADR-0049's admission that its hosting arm's bound reads an unauthenticated frame (measured: an authored frame suppressed the arm at `Clear` 10/10 over a live pool), and WP-060's planner coverage finding a live pool left standing by a prediction that removed its substrate. **The model already answered the question**: `ExtentLocator::Range` reads, verbatim, "a byte range within the host node's own address space", and `BackingExtent.host` names that node in a field the hashed address carries. Enforcement was what was missing. One comparison beside ADR-0046's now refuses the same disagreement — `BackingExtentFrameDisagreesWithName`, both frames named — while **absence still admits**, since a `Path`-located image has no contiguous device range and declares no extent. It does **not** put a backing extent into a containment forest: `named_position` still returns `Outside`, no pair-table row moves, `naming_referent_rule` is untouched, and ADR-0049's two measured-and-rejected routes stay rejected. Measured: three reds, **every one a repair**. ADR-0022's occupancy witness rebuilds on a lawful body with one field change and still proves the frame arm finds what nothing else does (`names_within` reaches no `Outside` node) — it had been built on a body whose name said one host and whose extent said another, which assembled only because of the carve-out, and that the arm survives honestly is the load-bearing result, since otherwise this act would have made ADR-0022's reading vacuous in issue #401's shape. ADR-0046's enumeration is **strengthened**, from `(17 × 20, 17 + 21)` to `(18 × 20, 18)`: the exception disappears and every node admits exactly one frame. ADR-0049's pinned limit closes. `crates/planner` took the consumer-first pull request its own grant required, retiring a witness whose body this act makes unlawful. Two mutations, each proven applied, each killed. **Minor under §0.1** on ADR-0041's and ADR-0046's precedent: what a body may say narrows, previously unspecified for this kind, and no requirement's text changes; no schema version moves and the golden vector is unmoved. Issue #365 closes entire — Part 1's two wrong doc comments corrected to state the relation rather than a list that can drift from it, Part 2's coverage delivered, Part 3 recorded as discharged by ADR-0045. |
| 17.0.0 | Reach follows the hosting name (ADR-0049, resolving issue #409). A `BackingExtent` names the node whose bytes carry it, and that field is in its hashed address — but `backing-extent` occurs in the endpoint-pair table exactly once, as the **source** of `HostBacking`, and as the target of no edge kind at all: measured, `Topology::build` refuses `containment(file-system → backing-extent)` outright. `affected_set` walks edges, so the relation the name asserts was one the closure could not traverse, and **the host-backed class had no upward reach where its extent is not in the frame of the step's own declared ranges** — the `ExtentLocator::Path` image framed on its file system, which is the loop and VHD/VHDX population, and any backing extent declaring no extent at all. A device-hosted `Range` backing extent was already reached by the byte seed; that narrowing was measured during this act's adversarial review and is stated rather than rounded up. Measured at `40e19d8` on a body that validates: wiping the disk that holds a loop image reached the disk and its file system and stopped, leaving the image, the volume, its signature and a live ZFS pool unreached, at `Clear` on **10 of 10** mutating operations — and the same for the **file system** target, which the filing had not recorded. **The closure gains a fourth arm**: a backing extent whose `host` names a node in the set is descended into, under its own bound — absence admits, an extent framed on the host admits, an incomparable frame admits, and the one refusal is a positive geometric contradiction — descending only, and carrying destruction exactly when the host is destroyed. Review caught the first form reusing containment's absent-child carve-out, which left the arm **inert on the very population it was written for**, a `Path`-located image declaring no extent; absence now admits. It reads the **name** to decide whether the relation exists, which needs no answer to what frames a backing extent; it reads the frame only to refuse a positive contradiction, and **that clause is unauthenticated for this kind**, so an authored frame can still suppress the arm at a measured cost of `Clear` 10/10 — pinned as an open limit under issue #365 rather than claimed absent. **Both alternatives were built and run, not argued.** Admitting `backing-extent` as a containment target closes the defect at zero reds but makes the honest body **unrepresentable** (ADR-0041 compares the image's extent to its file system's across frames and refuses), and reframing to satisfy that makes a 512 MiB image file read as destruction of the whole disk — false in principle, since a file has no contiguous device range, which is why `ExtentLocator::Path` exists. Additionally making the naming rule `Sources(CONTAINMENT)` costs **five reds**, including the single committed witness keeping ADR-0022's occupancy reading killable by mutation and ADR-0046's own carve-out pin. The chosen route costs **zero reds**; `crates/capability` and `crates/planner` are green unchanged. **Major under §0.1**: §2.1's closure sentence enumerates the arms and this adds one, the same shape as 13.0.0, 14.0.0 and 15.0.0. **ADR-0018's theorem is re-proved, not carried**: its premise is enumerated over the edge taxonomy, and this arm has no edge and does target a kind that may declare an extent — it is sound because that premise protects the *unbounded* propagating arms, while this one is geometrically bounded like containment, and the membership sentence gains a fifth parenthetical saying so. Four mutations, each proven applied, all killed — one only after the round added the regression it was missing, since reach-versus-destruction across the new arm is invisible until a partition table sits inside the image and a release can separate them. |
| 16.0.0 | An extentless target is destroyed by identity (ADR-0048, resolving issue #392 and closing ADR-0044's named limit). A `Volume`, `Aggregate`, `EncryptionLayer` or `MultipathNode` declares no extent, so `canonical_ranges` gave it **no destroyed range at all** — measured on the committed `partitioned_mdraid` fixture, a complete ADR-0046-lawful body with nothing omitted: `destroyed 0, written 0, consumed 0`, `affected = {md0, table}`, and `Clear` on **10 of 10** mutating operations over a live ZFS pool, for both `Wipe(md0)` and `Wipe(array)`. Every layer behaved as specified; the entry was empty, so the target was never destroyed, so destruction never carried, so the table was never destroyed and ADR-0043's release never fired. **Two rules land together, and the measurement is what establishes that both are needed.** *The canonical destroyed entry of an extentless target is its whole frame* — every byte in the target's own address space and, by `HostRange::intersects`' frame equality, none in any other — which range-destroys what is framed *on* the target and closes the volume; and *the destroyed-target seed gains a second source*, an extentless target named by a destroyed range framed on itself being destroyed by identity, which is the only thing that closes the **aggregate**, since nothing is framed on an aggregate and its content is framed on the volume it produces. The filed candidate was the first rule alone and left `Wipe(array)` `Clear` 10/10. Both together: `md0` and `array` move to `Clear` 6/10 — the four destroying operations refuse, the six that write and destroy nothing are deliberately unchanged — while the committed `Label(sda)` reach-is-not-destruction control and the `Wipe(sda)` control are **byte-identical**. Cost: one red workspace-wide, ADR-0044's pinned limit row, rewritten in place. **Major under §0.1**, and the amendment is the *seeding* change rather than the entry: §2.1's seeding sentence gains the second source and ADR-0018's theorem membership sentence gains a fourth parenthetical in the 15.0.0 shape. **The rejected minor reading is recorded**: the whole-frame entry by itself falsifies nothing — a volume's frame holds only its own content, and ADR-0042's target exception already licenses a target's own address space as a descent source — but a change to *when a node is destroyed* is a membership change, not an addition. Five mutations, each proven applied; three killed, one killed only after the round added the regression that catches it (the seed is **frame-equal**, not merely non-empty — invisible through the gate, since `canonical_ranges` always frames the entry on the target, and pinned at the closure where a plan step's own ranges reach it), and one recorded as a survivor that is a proof (the absent-extent guard is redundant on every constructible body, because the only extent-bearing target framed on itself is a frame root, whose self-extent has already fired the first clause). **Editorial, and not this act's claim**: "the operation's minimal invariant ranges" is **not** ADR-0018 text — it is ADR-0042:23's gloss, repeated in `capability.rs` and there miscredited to "ADR-0018's canonical-step rule", which issue #392's filing inherited; ADR-0038 had already made the entry non-minimal for `Shrink` and `Move`. The comments are corrected to state conservatism instead, along with a create-writes-and-consumes claim that had contradicted the code beneath it since ADR-0042. **What this does not do**: it does not close issue #319, whose third measured shape is pinned as an open limit so that closing it stays deliberate; it does not decide what frames or hosts a backing extent (#365, #409); and it does **not** discharge the planner simulation coverage this population lacks, which is WP-060's own pull request and this act's named obligation. |
| 15.3.0 | Inheritance ascends the name (ADR-0047, resolving issue #397 as filed). `device_scope_verdict` and `producer_verdict` both walked the **edge set only**, so a body that names its host in `FileSystem.host`, `BackingSignature.host`, `PartitionTable.parent` or `Partition.parent_table`, or its producer in `Volume.producer`, **with the edge omitted, inherited nothing** — measured on ADR-0045's own pinned named limit, where an xfs naming a multipath node gates `Clear` on all ten mutating operations with the edge dropped against `Unsupported{InheritedDeviceScope}` ×10 with it present, and likewise for a `RecognizedRemote` device's transport arm and for a volume naming a ZFS aggregate as its producer. ADR-0043 closed exactly this escape for *release* by reading `Partition.parent_table` rather than the edge; the inheritance verdicts never followed. **They follow now**: the containment parent a node's own name declares is ascended alongside every incoming containment edge, and the producers its name declares are folded alongside the `Production` and `HostBacking` edge sources — the qualifying fields read off `naming_referent_rule` rather than a second copy of the list beside the predicate. Both verdicts still fold with `worst`, so an added ancestry can only ever **add** refusal and a body whose name and edges agree answers exactly as before; the justification is ADR-0043's, generalized — an edge is authored content a body may omit, while a naming field is in the node's own hashed address, so omitting it changes the node rather than hiding its host. `affected_set` is untouched, no descent bound moves, the edge set is unchanged, and ADR-0018's non-interference premise is therefore not disturbed. A comparison-and-refusal shape was rejected: an absent edge asserts nothing to contradict, and refusing it would reject the honest partial captures 13.1.0 admits and price as a body-format change. Measured: **one red across the whole workspace**, ADR-0045's pin itself, whose revisit conditions name this rewrite; the producer half cost zero additional reds; `cargo xtask ci` exit 0 at 673 live tests. Six mutations, each proven applied — three killed, and three recorded as **survivors that are proofs**: the catch-all referent rule is unreachable for any field `naming_referents` emits (and `Topology::build` would refuse such a node anyway), every delivered rule's kind list is homogeneous so `.all` and `.any` agree row for row, and no kind with a non-`Permitted` `own_arm` can be a containment intermediate. **Minor under §0.1**: no requirement's text narrows — §2.1 fixes the per-node verdicts by reference to ADR-0018's evidence contract and delegates its content, and this extends that contract's device-scope decision to name-declared ancestry. No schema version moves and the golden vector is unmoved. **Named limit, stated rather than left to be found**: `NamingFields::Aggregate` carries no naming referents, so a signature whose `Backing` edge to its aggregate is omitted still leaves the aggregate unreached — this act does not close the omitted-edge escape in general. Issues #319, #392, #365 and #409 are untouched. |
| 15.2.0 | The frame rule is enforced (ADR-0046, resolving issue #333 and issue #401). ADR-0037 (12.14.0) decided that **a range in a containment forest is expressed in that forest's root address space** and held its enforcement for want of a green form and a capture-side referent sweep; ADR-0045 delivered the sweep. **The rule is now enforced, derive-and-compare, at `TopologySnapshot::assemble` and therefore at every decode**: for every extent a body declares, the containment root the node's own name leads to is derived — the one naming field per kind that names a containment source, followed until a kind that names none (a physical device, a volume, a multipath node) — and compared with the declared `host`; a mismatch refuses the body with both frames named, and the declared host is never replaced. **A containment edge agrees with the name**: an edge that nests a node in a parent its name does not embed refuses the body with both parents named — the strength ADR-0045 held beside this issue — so a node's three positional claims (name, edge, extent) are pairwise consistent, and ADR-0041's "incomparable frame" branch is discharged. A backing extent, which appears in no containment pair and whose `host` is the one open naming field, is outside every forest and is not reached (issue #365). Building it found and closes issue #401: ADR-0022's truthfulness precondition read occupancy as *frame naming*, which the accepted rule makes vacuous on every lawful capture — a partition is never a frame — so a decayed reversal bound; **occupancy is now read as bytes**: an extent framed on the host (kept), an extent lying on the host's bytes in the frame the host's own extent is expressed in (a region translated through it) with the host's own frame ancestors excused, and for the whole-host form a node whose own name positions it inside the host, extent or none; a host whose own extent is absent fails closed. Measured: issue #333's flagship defeat is unrepresentable on both construction paths, equal by value, and with every table edge removed; an enumeration over one body holding a device forest, a volume forest and a multipath forest at every depth admits exactly one frame per node (340 refused, 38 admitted); every containment edge re-sourced onto every other node refuses (59 by the name, 245 by the pair table); six committed layouts validate; the golden vector is regenerated in the same act — two of sixteen entries move by exactly `extent_host` and `extent_start`, the TypeScript suite unchanged — under MODEL-003's explicit-rejection limb with the schema version left at 1, the debt ADR-0037 said travels with the enforcement discharged. Fifteen mutations, each proven applied, each killed but one whose premise is now unconstructible, recorded. **Minor under §0.1**: the rule is 12.14.0's and §0.1 prices requirements, not enforcement; the edge-name agreement, the occupancy readings and the Section 5 sentence are additions; what a body may say narrows, as under 13.1.0. **It still does not make the reach sound**: every frame boundary remains safe only where a destroyed seed exists. |
| 15.1.0 | Names are admitted where edges are (ADR-0045, resolving issue #354's kind half). Eight node kinds embed a referent in their hashed name; PR #362 made every referent resolve, and a referent that resolved to the *wrong kind* — a partition whose `parent_table` is the physical device, a volume produced by a partition — still built at every boundary, which is ADR-0037:146-150's stated harm and the precondition #333's frame enforcement is held on. **Every naming referent must now resolve to an entry whose kind the endpoint-pair table admits as the source of the relation the field names** — containment for a table's parent, a partition's table, a signature's or file system's host, a conflicting entry's table; backing for an encryption layer's signature; production or host-backing for a volume's producer — a map from field to relation, never a second list of kinds, so a row added to the table admits the name in the same act; a backing extent's `host` is the one open field, no edge kind targeting a backing extent, and must only resolve. The check runs at construction and therefore at every decode. What held it after ADR-0044 was one population: content hosted on a multipath node, which no row admitted — measured to be an omission and a fail-open, an xfs naming `/dev/mapper/mpatha` building with no edge to carry it and every mutating gate `Clear` over a device §2.1 says never to mutate. **The pair table gains `multipath-node → {backing-signature, file-system, partition-table}`**, and content on a multipath node inherits the node's detection-only refusal through the ordinary device-scope ascent. Measured: the workspace green with the check on, the golden vector and the planner's rebuilds included; the naming enumeration admits 17 pairings and refuses 60; the five honest layouts earlier candidates false-refused all build with their edges; seven mutations, each proven applied, each killed. MODEL-003 under the explicit-rejection limb, schema version unchanged, on #362's reasoning. **Minor under §0.1**: MODEL-002 and Section 5 gain additions, the §2.1 multipath entry gains its content clause, and no existing sentence changes meaning. Two limits pinned and filed: device scope ascends the edge set, so a body that omits the multipath edge still gates its content `Clear`; and a table inside a partition (a BSD disklabel, an image written to a partition) is expressible by no row and therefore refused as a name — unrepresentable, not fail-open. |
| 15.0.0 | Destruction carries through the cascade, and a volume carries a partition table (ADR-0044, resolving issue #360). The endpoint-pair table omitted `volume → partition-table` — the missing third of a set two-thirds present — so a partitioned mdraid array and a GPT inside a LUKS-mapped volume had no representation. ADR-0043 measured what the row alone would ship: wiping the array's member disk descended four hops and stopped at the table, because reach is not destruction and only a step's own target released; it cut that propagation as an uncovered clause and named it this issue's remainder. **Destruction now carries: a step whose own destroyed ranges reach its target destroys the target; destruction is carried from there along the same arms, under the same geometric bound, as reach; every destroyed node releases what its name says it describes — a partition table its partitions, every other kind nothing — and a released partition is destroyed in turn.** Seeded by the target alone, never by a range that merely touches another node and never by reach. The modelling: a partitioned aggregate is `aggregate → volume → partition-table` over the production hop; aggregates stay out of the containment forest. Measured: the chain refuses the member wipe through the pool (`CCCCCCCCCC` → `CCRRCCCRCR` on the member, the array's superblock and the table), the GPT-in-LUKS layout refuses the LUKS partition's wipe with the ESP beside it 10/10 `Clear`, a partitioned array carrying a plain ext4 constructs, every existing layout byte-identical; seven mutations, each proven applied, each killed. One named limit, pinned: an extentless target declares no destroyed range, so its own wipe is not seen destroyed and reaches the table it carries as content only — the whole-frame canonical entry that would close it was measured green and held, filed, because it moves `canonical_ranges` and the planner's simulation on an uncovered population. **Major under §0.1**: §2.1's release clause and ADR-0018's theorem as amended in 14.0.0 both said the release follows the step's *target*, and both now say the target and the destruction carried from it; MODEL-002 gains the sentence that a volume may carry a table, an addition. |
| 14.0.0 | A destroyed partition table releases the partitions it describes (ADR-0043, resolving issue #347 on its third round). ADR-0018 defines the destroyed class as releases — content that "ceases to be referenced" — and the delivered closure reached none of the partitions a destroyed table describes: on a disk carrying a live ZFS vdev, `Wipe(table)` was 10/10 `Clear` with the pool never consulted. Two designs died inside the closure on the table's own authored geometry (round 1's coverage test fail-open on one byte of inflation; round 2's intersection test capturing every sibling from a partition-target step), and round 2's panel measured that no predicate over `Facts.extents` can be repaired into shape. **The release is now structural: a step whose target is a partition table and whose own destroyed ranges reach it destroys the table, and a destroyed table releases every partition whose name says it describes it** (`Partition.parent_table`, on the roster the topology already sweeps) — never a table some other step's range happens to touch, never by coverage of the table's extent, never by the edge set; a `ConflictingTableEntry` names a table and is not released by it. Measured on the round-2 panel's own fatal shapes: its L1 partition-target row is 10/10 `Clear`, its L2 one-byte inflation no longer refuses, its M3 hybrid-MBR wipe stays `Clear`, its M5 omitted-edge escape is closed; the plain disk still constructs. One priced limit, pinned rather than hidden: a table-target step destroying any byte the body attributes to the table is read as destroying it, fail-closed. **Major under §0.1**: ADR-0018's non-interference theorem as amended in 13.0.0 is amended again, and §2.1's closure sentence gains the release clause and is corrected to carry ADR-0042's target exception — which that act changed the meaning of and priced as unchanged; the mispricing is corrected here rather than smoothed over. Six mutations, each proven applied, each killed. The #360 chain (a table on a volume, the member disk wiped) needs the release to propagate through the cascade; measured to work and cut from this act as an uncovered clause, recorded as #360's remainder. |
| 13.1.0 | The body's facts are validated against its topology at assembly (ADR-0041, resolving issues #349 and #356 as filed). A topology snapshot's evidence-contract facts — extents, transports, member counts, table states — were validated by nobody: `assemble` stored them verbatim, the decode path checked an extent only for kind-misplacement, and the in-process constructor applied none of the decode path's checks, so the planner and the capability engine could hold a snapshot the body boundary would refuse. **Six rules now apply at `TopologySnapshot::assemble`, the one path both the in-process constructors and the decode boundary run through**: every fact is keyed by an absorbed entry whose kind carries it; an extent's `host` is an absorbed entry, its length at least one byte, its `start + length` below `u64::MAX`; and a containment child's extent lies within its parent's **where the pair is geometric and the frames are comparable** — `partition-table` → `partition` and → `conflicting-table-entry` are structural, the table's extent being its own header bytes and not the region it governs (measured: read as geometric, the rule refuses every committed GPT disk and a pre-existing step test), while the seven other Containment pairs carry a span claim. A body whose two positional claims contradict is **refused, neither preferred**. Deliberately left alone, each named in the ADR: a child in a frame its parent cannot be compared against (ADR-0037's held enforcement, issue #333); a child whose parent declares no extent (the golden vector's shape); an absent extent (honest absence — issue #356's absent-extent spelling was re-measured under the act and **still constructs**, which is issue #319's class and is not claimed); sibling non-overlap; a device's extent against its own `total_bytes`. **Minor under §0.1**: what a body may say was previously unspecified territory, on ADR-0037's own pricing, and no existing requirement's text narrows. Under MODEL-003's explicit-rejection limb with `SCHEMA_VERSION` left at 1, on PR #362's precedent — the refused population was never lawful, only unvalidated, and the golden vector and every previously committed test but one are unmoved; that one, WP-060's occupancy test, built two refused shapes on purpose and was adjusted first under its own grant. Twelve mutations, each proven applied, each killed. The overlapping-geometry fixture the issue-347 round-2 panel required is committed with its `f11`/`f12` assertions. |
| 13.0.1 | The relocation exemption is retired, and the release entry stands (ADR-0040, resolving issue #348). ADR-0018:141-145 exempted "the relocated target's own subtree from destruction descent"; ADR-0038 gave `Move` the whole-target-extent `destroyed` entry that captures exactly that subtree. **The exemption is retired as void where it stood** — §0.2's rule 4 forbids an ADR weakening a spec MUST and §2.1's enforcement paragraph is a MUST NOT — and it was additionally never delivered, never cited by any requirement, and not expressible in the delivered closure, which takes no `Operation` at either call site. **Patch under §0.1**: no requirement text changes, no reach moves, the closure is unchanged line-for-line, and a sweep finds no normative citation of the clause anywhere outside ADR-0018 itself; the no-bump and major counter-arguments are recorded in the ADR and declined. The issue's own option — reverting ADR-0038's `Move` entry — was **rejected on measurement**: moving `Move` between range arms changes no verdict on six targets with the suite green, and deleting its entry outright changes none on a partition while turning a whole-disk `Move` from `Unsupported{Zfs}` to **`Clear` over a live pool**, because on a disk target a self-framed extent is never a descent source and reach is therefore entirely range-driven. That mutation survived the whole committed suite, so the one regression this act adds is the coverage that was missing; it is proven to bite under both mutations rather than merely to pass. Also rejected, on structure read off the delivered types: a computed preservation predicate, which must key on `facts.extents` while no §2.1 aggregate can carry an extent, leaving it inert on exactly the class it must relieve. The byte-wise-preservation half of ADR-0018's paragraph survives as a plan-layer duty, delivered nowhere and filed. **The availability gap the exemption named is real and stays open, filed rather than settled** — a length-preserving relocation of a protected partition refuses although copy-then-commit would preserve every byte; the decision owner was put the alternative of recording that refusal as settled product policy and declined it. |
| 13.0.0 | Carried-content reach, and a bounded descent (ADR-0039, on issue #338's held half): **a mutating step's affected set closes over the content its target carries, not only over the substrate the step destroys**, and containment descent is bounded per edge target by declared geometry rather than by the destroyed ranges. **Major under §0.1**: this section's own closure sentence, ADR-0018's rule 2 and its Decision all said *bounded by the destroyed ranges*, and that bound is the defect — a ZFS label at `[512,513) MiB` is not inside a freed tail at `[640,768) MiB`, so a shrink truncating 128 MiB off a live vdev reached nothing and constructed, at the plan-body re-validation boundary that has no capability gate in front of it. Six operations that destroy nothing — `Grow`, `Create`, `Repair`, `Label`, `Uuid`, `Decrypt` — seeded no propagating class at all and gated `Clear` over a live pool; they refuse now by carrying reach from the target. The bound refuses a hop only where declared geometry positively contradicts containment and admits on every absence, mismatch or ambiguity, because extents are authored body content and a predicate that can subtract reach hands that content a lever: four earlier predicates were rejected on measured fatals, two for exactly that (one turned a live-pool refusal into `Clear` on a body whose node ids and hash are unchanged) and two for false-refusing ordinary disks. A node's own address space is never a descent source, which is what stops a device's whole-disk extent from carrying reach into its siblings, and the no-sibling-capture premise is generalized past the name `physical-device` and enumerated over the endpoint-pair table — MODEL-002's standing obligation that it be re-proved as a property, which no committed test discharged. |
| 12.14.0 | The containment-frame anchoring rule (ADR-0037, on issue #333): **a range in a containment forest is expressed in that forest's root address space**, and `HostRange.host` names that root. The anchoring was never validated — `assemble` stores facts verbatim and the decode path checks `extent_host` only for kind-misplacement — while `HostRange::intersects` opens on it, so it decides reach exactly; and three committed fixtures disagreed, the cross-language golden vector among them. Measured: re-anchoring one signature, with every extent still present, left a ZFS pool unreached and a whole-device wipe constructing. The accepted rule is the one ADR-0018's **safety analysis** already stated (never its Decision, and the delivered echo cites a section number that does not exist) and the one `Partition.start_offset`'s own doc already assumed. Rejected on measured fatals and recorded in the ADR: **host-named** (blind to partial destruction on a step the delivered planner already emits, and `Clear` at the capability surface for Shrink/Grow/Move — its textual pillar dissolving once `ExtentLocator::Range` is seen to govern a kind outside the containment forest entirely); **derived frames** (three safety fatals, though it produced the round's only positive evidence for the accepted rule); a **per-kind roster**; and enforcing by a **topology-derived root walk**, measured to break 14 committed tests and wrong in kind, since any target kind becomes a root the moment its containment edge is absent. **Enforcement is deliberately held** — none has been measured green; the naming-field-derived front-runner is recorded as a candidate with its cost, in derive-and-**compare** form only, since derive-and-replace was measured to delete facts the delivered types depend on. Priced knowingly: the rule makes the coordinates uniform but **does not make the reach sound** — every frame boundary stays a hole in the non-destroying arm — and the golden vector plus `plan_tests.rs` are unlawful under it until the enforcement PR regenerates them as a versioned act with its own MODEL-003 discharge. **Minor under §0.1**: an addition to previously unspecified territory, no existing requirement's text changing or narrowing; the **patch** counter-argument (a rule nothing enforces is editorial) is recorded and declined, because deciding which of two committed conventions is lawful is requirement-shaped whether or not anything implements it. Issue #319's authorization half is blocked on **issue #338** — eight of twelve operations seed no destroyed range, so the closure never runs — not on this. |
| 12.13.0 | The scheme's own regions, and located occupancy (ADR-0036, on issue #319): **a fail-open in the delivered free-extent derivation, measured rather than argued.** At `ecb3dc6` a `place_create` of exactly the 1 MiB default against the delivered solver fixture returned `start=0 … Aligned` — a partition over the protective MBR and the GPT header, recorded as conforming — because that fixture's partition-table node carries a containment edge and no extent and the subtraction never saw it. The fail-open had a committed witness asserting it was correct. **INV-004** gains two things: the free-extent derivation withholds the regions a host's declared table schemes claim at each end, stated as a **bound and never a measurement** (the fields fixing a scheme's exact regions — sector size, entry count, entry size — do not reach the derivation), and it is not presented at all where the host declares a scheme the build cannot name or where an authenticated partition of the address space is not one the derivation subtracts. **PART-009** gains a third named structural edge: the low boundary of a scheme-claimed tail region, so a grow filling a GPT disk is spelled coincident with the scheme's region rather than the device end — filling to the physical end overwrites the backup header. The claim derives from the table node's own hashed `TableRole`, so it is authenticated, not asserted; occupancy is read from the authenticated names and never from containment edges, which ride in no node's address preimage. Rejected and recorded in the ADR, both on **measured** fatals: keying the guard on extent *presence* (proves a key exists, proves nothing about the range it holds — the create was still placed over a buried occupant with every extent populated), and refusing outright on an unlocated table (manufactures a refusal from absence, the mirror of the defect); also rejected, requiring conflicting table entries to carry extents (bricks ADR-0024's repair family) and including hosted layers in the occupancy roster (a measured false refusal on a delivered fixture, squarely inside issue #333's open anchoring ambiguity — excluding them is what keeps this decision independent of it). Two further §11.2 defects close with it: a host extent exceeding the size its own name declares (measured — a 1 GiB device carrying a 2 GiB self-extent placed a 1.5 GiB partition, recorded `Aligned`) and a child extent leaving its host. **Minor under §0.1**: INV-004's derivation sentence and presentation prohibition stand verbatim and PART-009's list gains a third entry on the 12.1.0 precedent; nothing narrows. The **major counter-argument is recorded in the ADR and was put to the decision owner in its strongest form** — the tail bound withholds ~1 MiB where the backup GPT needs ≤132 KiB, and reading that under-reporting as narrowing INV-004's detect duty prices 13.0.0 — and 12.13.0 was held against it. Issue #319's authorization half stays open and is blocked on #333; this is its planner half only. |
| 12.12.0 | The Linux mmc-class designation extension (ADR-0035): **ADR-0034's revisit condition fired in its sanctioned direction the day it was written, and the table extension lands with its rows.** The (Linux, native MMC-attached block device) serial cell designates the **`cid` attribute of the mmc device node the block device's sysfs `device` link resolves to** — the full 128-bit register, bytes verbatim, trailing newline included; the attachment class is the S5c-measured structural resolution (the linked node under `mmc_host/*`, no ancestor search). The kernel-parsed `serial`/PSN projection is the recorded rejection: a transformation between the medium and its name with strictly weaker collision resistance — the S5 pair shares `manfid`, `oemid`, and manufacture date, so the non-PSN fields are survivable-collision context, not padding. WWN stays undesignated for the class; ADR-0034's `ObservedAbsent`, failed-read, bytes-path, and verbatim-newline rules apply unchanged; the host-assigned RCA — measured identical across two media and reassigned identically across reinsertion and reboot — is path material, never identity. Evidence base and its edges stated rather than smoothed: SD cards on one `sdhci-pci` host; eMMC is designated by interface contract without its own rows yet (a priced judgment — scoping by card type would make the class boundary read a field no naming rule consumes), with the first eMMC capture and a second-host capture as the ADR's evidence obligations. Rejected and recorded: the PSN projection, and waiting (undesignated cells are already fail-closed; the measured class is the register's strongest identity evidence and round five's route). **Minor under §0.1**: one previously undesignated cell gains a designation; no existing text narrows. SI-28, its floor, and its round five are untouched — these rows enable round five and do not decide it. |
| 12.11.0 | The Linux naming-source designations (ADR-0034): **ADR-0019's anticipated normative act is made, on measured rows alone.** The per-platform source table gains a (platform, attachment class) key — stated as an addition, preserving the rule's purpose of exactly one source per identifier per device — with exactly one designated cell: for Linux USB-attached block devices, the serial is the `serial` attribute of the device's nearest USB device-node sysfs ancestor, **bytes verbatim as read, trailing newline included** (stripping is a transformation with an undecidable edge, and ADR-0019 excludes the class wholesale). WWN and every other Linux attachment class are **undesignated**: fields absent, names weaker, collisions fail closed through the existing collision-group machinery — nothing is designated by prediction. Two naming outcomes ADR-0019 left undefined close in the same act: a **measured absence** (`ObservedAbsent`) joins `unavailable` — field absent, name weaker, the device operand-eligible; a **failed read** of a designated source (the 2026-08-04 sitting's `device/wwid` `ENXIO` is the measured shape) names the device from its remaining fields, marks it indeterminate, and blocks it as an operand — the unreadable-aggregate-designator posture extended to the recursion root, with detection continuing and the governing finding's whole-host denial impossible. Naming inputs MUST flow through a bytes-preserving path; the delivered `read_attribute`'s decoded, newline-stripped projection is not a lawful naming input, and the bytes seam is WP-L100 increment 3's first delivery obligation. Rejected and recorded in the ADR: udev `ID_SERIAL_SHORT` (a cached third-party computation, a divergence-capable second source, measured worse availability), sysfs `device/serial` (zero observations on any Linux host — the 2026-08-13 readback established the sitting never read it), `device/wwid` for WWN (would be born firing ADR-0019's own revisit condition), and holding for a complete table (undesignated cells are already fail-closed; the per-class extension is what the revisit condition schedules). **Minor under §0.1**: every change is an addition to previously undefined territory — no Linux designation existed to narrow, no numbered requirement's text changes. SI-28 is untouched and remains WP-L100 increment 3's register gate exactly as recorded. |
| 12.10.0 | Resolved SI-14 (ADR-0033): **a derived property is a derivation, not an observation.** MODEL-004's "discovered property" means an observed one; a property computed from other properties — INV-004's free extents and alignment are the two — is recomputed at use from the detected inputs it names, is never stored, and carries no observation set or confidence of its own: its trustworthiness is its inputs', which carry the observation sets. One new prohibition lands with the reading: a derivation over an input whose observation set derives `unavailable` or `conflicting` MUST NOT be presented as a value — the input's own state is surfaced instead, so a guess can never wear a computation's clothes. No fifth confidence value and no composition algebra: MODEL-004 confidence describes observation trust, not computational correctness, and a composed or tagged value would let a record assert what its observations do not carry — the stored-confidence shape ADR-C4 made unconstructible. The delivered architecture already embodies the rule (ADR-C4's constructor-absence proof; WP-060's solver computing free extents from body-carried authenticated extents; ADR-0023's rejected duplicate alignment-fact field), which is why the issue's "Later (WP-050)" gate had been reached and passed without a rule appearing: the absence *is* the rule, now recorded before a discovery package could implement the literal reading and mint observation sets for computed values. Presentation obligations for the WP-W100/WP-L100/WP-M100 INV-004 surfaces are recorded in the ADR and land in those assignments at creation. Rejected and recorded: a composition algebra (describes nothing MODEL-004 measures), a fifth `derived` confidence value (stored confidence by another name; the vocabulary-doubling shape ADR-C3 removed), and a synthetic computation-adapter observation (fabricated provenance, the duplicate-field agreement obligation generalized). **Minor under §0.1**: INV-004's duty survives verbatim, the clause specifies only how two list items are produced — never previously specified — and adds one prohibition; the major counter-argument (the 3.1.0 caution) is recorded in the ADR. SI-13, SI-28, and SI-37 stay open exactly as recorded, their gates verified by the same round's companion sweep. |
| 12.9.1 | Resolved SI-26 (ADR-0032): **Section 16's "Stable" is CAP-003's `supported` — a stale synonym, and the evidence rule already lives there.** Section 16's prohibition and CAP-003's definition ("`supported` — apply permitted, backed by matrix evidence (CAP-006)") are one rule seen from two sections; the word predates CAP-003's fixed vocabulary. No maturity axis: a second quality vocabulary orthogonal to status would double what every consumer handles for a qualified-but-immature state no requirement recognizes — the vocabulary-doubling shape ADR-C3 removed and ADR-0015's option (e) rejected — and it would weaken what `supported` promises. WP-050's delivered engine already makes the reading structural: `supported` is constructible only through CAP-006 qualification evidence whose token has no constructor until a qualifying row exists, so Section 16's prohibition is compile-checked today. The landing is a **patch**: Section 16's sentence gains the parenthetical "(that is, CAP-003 `supported`)" — purely clarifying under the selected reading, §0.1's editorial class, the ADR-0020 reading-selection shape with a one-phrase repair instead of a bare banner; the major counter-argument (the 3.1.0 caution against fixing meaning by edit) is recorded in the ADR and was not taken because the decision is the ADR's and the edit changes nothing under it. Rejected and recorded: the maturity axis, and renaming the word outright (retexts a prohibition sentence for what a parenthetical achieves editorially — the SI-20 row-rewording economy). CAP-003's four values stand verbatim, exactly as ADR-0020 and ADR-0026 left them. |
| 12.9.0 | Resolved SI-25 (ADR-0031): **CAP-002's list is a required minimum over a closed-and-versioned operation vocabulary.** The list was never a claim that no other operation may exist — PART-007 (split/merge), PART-010 (convert), and PART-011 (migrate) are existing normative operations the list predates, and they join the vocabulary as named operations when their packages build them. The vocabulary is closed at every moment and moves only by reviewed, MODEL-003-versioned spec change — minimum-over-time, closed-in-the-instant, which is what keeps CAP-005's one-engine promise stable while the floor reading keeps DIA-005 implementable. **`wipe` is a family**: DIA-005's six kinds (overwrite, crypto-erase, sanitize, format, discard, file deletion) become separate operations when erase surfaces are built — capability genuinely differs per kind (sanitize needs DIA-004's device checks, discard needs TRIM, crypto-erase needs an encrypted layer), so separate modeling makes never-equivalent structural rather than behavioral; the kind-discriminated alternative was rejected as equivalence at the modeling layer, the thing DIA-005 forbids. The delivered `crates/capability` `Operation` enum stands as-is until WP-050's next reviewed increment extends it under this discipline — the standing debt pattern, recorded rather than tripped. Rejected and recorded in the ADR: the closed-enumeration reading (makes DIA-005 unimplementable and required operations unrepresentable — fail-closed posture spent making features impossible) and the open minimum without versioning (surface drift against CAP-005). **Minor under §0.1**: CAP-002's sentence stands verbatim; the floor, versioning, and family rules are additions; DIA-004, DIA-005, PART-007/010/011, CAP-003, and CAP-005 untouched. SI-13 and SI-14 stay open. |
| 12.8.0 | Resolved SI-23 (ADR-0030): **the REC-011 backup is a first-class protection artifact with four rules.** The object the spec mandated and then said nothing about — a LUKS header backup freezes key-slot state, so a passphrase revoked after the backup remains usable with it — gets its home, its reference discipline, its retention, and its end of life. **Home:** a dedicated helper-owned protection-artifact store inheriting JRN-004's admin-protected documented-location clause, sibling to and never inside the journal — JRN-005's bounds stand, ADR-0029's budget is not bloated with bulk bytes, and replay never drags key-slot material past every consumer; this answers ADR-0029's named fork in terms: the lifecycle does not route through the journal. **Reference by identity:** the journal, the plan (Section 6's existing backup-and-recovery-actions body item), and every SAFE-006 surface carry only the artifact's content hash and store identity — SAFE-006's list stands verbatim, the artifact joins none of its surfaces, and only the helper reads the store (SAFE-008's discipline); a restore is an ordinary identity-validated plan (REC-001, REC-011's PART-013-shaped binding) at its own authorization tier. **Retention:** ADR-0029's liveness rule, adopted in REC-011's own text — the artifact is retention-exempt while its creating apply or any apply whose linkage closure references it is non-terminal, which is the filing's "RecoveryAction must reach it" made structural. **End of life:** after the closure terminates, explicit user-controlled retention in SEC-009's shape, never silent in either direction — the deciding surface states both costs (retention preserves revoked-passphrase slots; deletion forfeits the only disaster-recovery copy), with displayed, changeable defaults permitted and silence forbidden. Rejected and recorded in the ADR: embedding in the journal (three disciplines traded for an inheritance the store gets by citation), a user-chosen arbitrary location (unprotected key-slot material, unreachable recovery), and auto-delete on completion together with its silent-retention mirror (both silences rejected as one). ADR-0024's corrupt-source discharge stays WP-R100's, untouched. **Minor under §0.1**: REC-011's two sentences stand verbatim, the four rules are additions, and SAFE-006, JRN-004, JRN-005, SEC-009, REC-001, and Section 6 are untouched. The store's layout, encoding, and per-OS paths land with WP-R100/WP-070, jointly sequenced; no re-attribution follows — neither assignment exists, and the ADR records the verification obligations so their creation cannot omit them. |
...
Enforcement of these MUST NOTs is representational, not merely checked (ADR-0012): a mutating plan step whose target resolves to a node protected by this section is unrepresentable in the plan type — constructing it is a type error, not a validation failure — and the helper's independent recomputation under HLP-002 remains as an unweakened second layer. The closure that decides *which* nodes this section reaches is fixed by ADR-0018 (resolving SI-11): per-node protection verdicts are three-valued and total with an `Indeterminate` residual — never `Permitted` by default — computed from ADR-0018's named two-layer helper evidence contract; a mutating step's affected set closes over the substrate it destroys **and over the content its target carries** (downward containment, upward backing, downward production, and downward hosting — a backing extent whose own name says a node in the set carries its bytes, a relation the endpoint-pair table can express on no edge and the name therefore carries alone *(added in 17.0.0 by ADR-0049; bounded by the same declared geometry as containment)*), with a released range counted as destroyed; descent is **bounded per edge target by the declared geometry** — refused only where a child's declared extent positively contradicts containment within one frame, and admitted on every absence, mismatch or ambiguity, since those extents are authored body content and a bound that can subtract reach is a lever on protection — with the one exception the pair table itself states: where a containment pair is *structural* rather than geometric, the parent's extent being its own bytes and its children lying beside them rather than within, descent is refused whether the child declares an extent or not, which is the same answer the geometric comparison gives when it does *(the precision added in 17.2.0 by ADR-0051, which also made the geometric case admit on absence as this sentence had always required)*; a node's own address space is never a descent source except for the step's own target *(ADR-0042; the exception was priced as unchanged there and is carried by 14.0.0)*, so a device's whole-disk extent cannot carry reach into its siblings; a step whose own destroyed ranges reach its target destroys the target — and a target that declares no extent, named by a destroyed range expressed in its own address space, is destroyed by identity, since a range cannot reach a node that states no position *(added in 16.0.0 by ADR-0048; the canonical entry for such a target is its whole frame, which by frame equality touches no other)* — destruction is carried from there along the same arms and under the same bound as reach, and a destroyed partition table — the step's target, or one the carried destruction reaches — releases every partition whose name says it describes it, by the naming relation and never by the table's declared extent or by any containment edge, a released partition being destroyed in turn *(added in 14.0.0 by ADR-0043; the carriage from the target, and the release it triggers, in 15.0.0 by ADR-0044)*; and a step whose affected set reaches a node this section protects is the unrepresentable sentence, while an `Indeterminate` member refuses construction with a typed artifact. Table writes target the table node's own extents, never the parent device wholesale. What keeps a protected member's siblings unconstrained is that rule together with the descent bound above and the edge taxonomy's own premise — no backing, production or host-backing edge may target a kind that declares an extent — enforced as an enumerated property of the endpoint-pair table rather than asserted.
...
- **Multipathed attachments (SAN/MPIO/dm-multipath):** detect and represent via the platform's own multipath framework; never mutate a multipath device or a recognized member in v1 (ADR-0011), nor content — a signature, a file system, a partition table and what it describes — hosted on a multipath device, which is represented and inherits the device's refusal *(added in 15.1.0 by ADR-0045)*. Two block devices presenting equal stable identifiers with no platform-assembled multipath node are SAFE-005 ambiguity — mutation on either reports `blocked`, refused without asserting the two are one device.
...
- **RPC-001:** Transports: Windows — named pipe with an SDDL restricting access to SYSTEM and the authorizing interactive user; Linux — Unix domain socket in a root-owned directory searchable but not writable by others (mode 0711), the socket node owned by the authorizing user and accessible to that user alone (mode 0600), with peer-credential verification of the connecting process against that user (ADR-0055); macOS — XPC with code-signing requirement checks, or an equivalently verified Unix domain socket.
...
always encodes regardless of on-disk content. A referent a naming map embeds
MUST be the address of an absorbed node whose kind the endpoint-pair table
admits as the source of the relation the field names — a partition table's
carrier, a partition's table, a signature's or file system's host, an
encryption layer's signature, a volume's producer, a conflicting entry's
table — checked at construction and therefore at every decode; a backing
extent's host, which no edge kind targets, must only resolve. *(Added in
15.1.0 by ADR-0045, on issue #354; the resolve half landed unversioned in
PR #362 under MODEL-003's explicit-rejection limb.)* An extent a body
declares for a node in a containment forest MUST be framed on the
containment root the node's own name leads to, and a containment edge MUST
nest a node in the parent its name embeds — both checked at construction
and therefore at every decode, each refusal naming both claims; a backing
extent, which appears in no containment pair, is outside every forest and
its range lives in its host's own address space. *(Added in 15.2.0 by
ADR-0046, on issue #333; the rule itself is ADR-0037's, 12.14.0.)*
...
It MUST also represent non-linear relationships such as Storage Spaces, LVM, RAID, APFS containers, and Btrfs multi-device file systems. These are modelled as follows. *(Added in 4.0.0 by ADR-C5.)* The chain lists the layers, not a single nesting order: a volume may itself carry a partition table — a partitioned mdraid array, a GPT inside a mapped volume — and the chain re-enters at *partition table* below it. A partitioned aggregate is `aggregate → volume → partition table` over the production hop, the table's parent being the produced volume, never the aggregate; the endpoint-pair table admits `volume → partition-table` beside `volume → backing-signature` and `volume → file-system`, and no aggregate carries a table of its own. *(Added in 15.0.0 by ADR-0044.)* A platform-assembled multipath node carries content the same way a volume does — a signature, a file system, or a partition table — and the endpoint-pair table admits `multipath-node → backing-signature`, `→ file-system` and `→ partition-table`; content so hosted inherits the node's detection-only refusal (Section 2.1, ADR-0011) through the ordinary device-scope inheritance, exactly as content on a physical device inherits its transport arm. *(Added in 15.1.0 by ADR-0045.)*
...
- **INV-004:** Detect partitions, free extents, alignment, partition types/flags, labels, UUIDs, volumes, file systems, encryption, mounts, and nested storage. Free extents and alignment are **derivations, not observations** (12.10.0, ADR-0033): each is computed at use from the detected inputs it names (partition extents, device geometry), is never stored, and carries no observation set or confidence of its own — its trustworthiness is its inputs', which carry the observation sets (MODEL-004). A derivation over an input whose observation set derives `unavailable` or `conflicting` MUST NOT be presented as a value; the input's own state is surfaced instead. The free-extent derivation additionally **withholds the regions the table schemes a host declares claim at each end**: the scheme is a detected input (INV-003) carried in the table node's own authenticated name, and because the fields fixing a scheme's exact regions — sector size, entry count, entry size — are not available to the derivation, the withheld region is a **bound and MUST NOT be presented as a measurement**. The derivation MUST NOT be presented at all where the host declares a table scheme the build cannot name, or where a partition the authenticated names place in the host's address space is not one the derivation subtracts; the inputs' own state is surfaced instead, on the same ground as the clause above. *(Amended in 12.13.0 by ADR-0036, on issue #319: the derivation sentence and the presentation prohibition stand verbatim; the withholding, the bound rule, and the two additional unavailability grounds are additions.)*
...
- **CAP-002:** Model detect, read, create, grow, shrink, move, copy, check, repair, label, UUID, encrypt, decrypt, and wipe separately. This list is a **required minimum**, not a closed enumeration (ADR-0031, resolving SI-25): the operation vocabulary is closed and versioned at every moment (MODEL-003) and extends only through reviewed spec change, never by drift. `wipe` is a family whose DIA-005 kinds — overwrite, crypto-erase, sanitize, format, discard, file deletion — are modeled as separate operations when erase surfaces are built, making never-equivalent structural; PART-007's split/merge, PART-010's conversion, and PART-011's migration join the vocabulary as named operations when their packages build them, because their feasibility is not derivable from member operations. *(Amended in 12.9.0 by ADR-0031; the first sentence stands verbatim.)*
...
- **LIN-001:** Discover through the kernel's and the distribution's client-readable interfaces — sysfs, the udev database, procfs, and `os-release` — each interface entered only by a measured row (ADR-0054); UDisks2 is not the client's discovery interface. UDisks2, libblockdev, or authoritative native tools for authorization and mutations, behind the helper's own recorded route decision (WP-L110; HLP-002, LIN-009).
...
- **REC-011:** Before mutating an encryption layer, create and verify a backup of its metadata (LUKS header, BitLocker metadata) with the same identity binding as PART-013. A failed or unverifiable backup blocks the operation (SAFE-005). The backup is a **protection artifact** under four rules (ADR-0030, resolving SI-23): it lives in a dedicated helper-owned store inheriting JRN-004's admin-protected documented-location clause, sibling to and never inside the journal; the journal, the plan, and every SAFE-006 surface reference it by content hash and store identity only — its bytes join none of those surfaces, only the helper reads the store (SAFE-008), and a restore is an identity-validated plan (REC-001) at its own authorization tier; it is retention-exempt while its creating apply, or any apply whose linkage closure references it, is non-terminal (the ADR-0029 liveness rule, adopted here); and after that closure terminates it falls under explicit user-controlled retention in SEC-009's shape, never silent in either direction — the deciding surface MUST state that retention preserves the key-slot state at backup time (a passphrase revoked since then remains usable with the artifact) and that deletion forfeits the disaster-recovery copy, with displayed, changeable defaults permitted and silence forbidden. *(Amended in 12.8.0 by ADR-0030; the two pre-existing sentences stand verbatim, and the corrupt-source discharge question stays with ADR-0024's revisit condition.)*
...
| Primary | Debian / Ubuntu | Debian 12 / Ubuntu 22.04 LTS; kernel ≥ 5.15 | Full advertised matrix |
| Primary | Arch Linux | Current rolling | Full advertised matrix, tool-version-gated |

Per-tool version floors live with the capability fixtures (CAP-006) in `docs/capabilities/`, not in this spec. UDisks2 ≥ 2.9 is such a floor (ADR-0054; until 18.0.0 it sat in the Debian/Ubuntu row): entered in that store by the first package that invokes UDisks2, gating the operations that use it.
...
- Mark a capability Stable (that is, CAP-003 `supported`) without its matrix fixture and acceptance evidence.
