...
`AGENT_BUILD_SPEC.md` 13.0.0. The intended product is a dark-first Tauri desktop
...
Twelve are open as of `6d743a3` (2026-08-15). This section reports status
...
**One obligation is outstanding before any of them.** A non-Markdown path
(`crates/domain/src/model/protection_tests.rs`, from PR #372) landed after
WP-020's r20 pin at `6d4a8fc`, so the stopping condition at
`docs/work-packages/WP-020.md:1202` is tripped and **an r21 sitting is
owed** on `6d743a3`. The check is
`git diff --name-only 6d4a8fc HEAD | grep -v '\.md$'`, and it must be run
