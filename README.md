...
**No sitting is owed at the time of writing.** Issue #409's act (PR #419 — ADR-0049, spec 17.0.0) shipped Rust after the r30 pin at `55e5601`; the r31 sitting (VMID 9455, 2026-08-17 UTC) on its merge commit discharged it, and WP-020 is re-pinned at `8791a56`. Before it: ADR-0048's planner coverage (PR #416 — WP-060, test-only), whose r30 sitting (VMID 9454) discharged its own trip, and before that issue #392's act (PR #414 — ADR-0048, spec 16.0.0), and before that issue #397's act
...
check is
`git diff --name-only 8791a56 HEAD | grep -v '\.md$'`, and it must be run
