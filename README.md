...
**Its acceptance passed operator-run on 2026-08-11, twice** — first on
`4fbb2f9`, then re-taken the same day on `68298f2` after the #248/#249/#250
review-finding fixes changed its own probe and write lines — each time in a
disposable Proxmox-hosted non-WSL VM, in the same sitting and on the same
commit as a re-take of the increment 2e read-only acceptance. The kernel
