...
is denied as on the other two platforms. **M10, the privileged comparison
leg, is the one preregistered cell still `not yet taken`** — it needs a
disposable macOS VM — and M9's Fusion shape is `not established` because
Apple Silicon has no such hardware. The macOS second-reader readback is
outstanding.
