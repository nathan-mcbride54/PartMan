...
that environment. **Issue #94 closed on 2026-08-03**, so that rerun is now
unblocked — the preregistered hardened protocol may run. Nothing already
measured improves: the loop criterion stays unsatisfied until the rerun is
actually taken, and a closed gate is not a result. Increment 6's macOS and real-partitioned-Linux matrices,
