...
`AGENT_BUILD_SPEC.md` 17.4.0. The intended product is a dark-first Tauri desktop
...
**No sitting is owed at the time of writing.** WP-L100 PR #493 — increment 5a of the Linux read-only inventory adapter, the capability seam: WP-050's CAP-004 `RuntimeFacts` in the engine's own vocabulary, an empty tool roster for every served operation, a typed refusal for mutating ones, the ACC-009 mapping fail-closed, nothing launched — shipped four non-Markdown paths under `crates/adapter-linux/` (plus the regenerated `Cargo.lock`) after the r46 pin at `199bb06`; the r47 sitting (VMID 9476, 2026-08-19 UTC) on its merge commit `e1f138b` discharged it, named in the PR body before merge, and WP-020 is re-pinned at `e1f138b`. No invocation was void; the guest was created and settled before the PR merged and provisioned on its merge commit, and never rebooted. A green run says nothing about the seam's correctness — that evidence is its tests and its five killed mutants. Before it: WP-L100 PR #490 — a module-doc correction in the Linux read-only inventory adapter, doc comments only (the held report's consumer is the helper capture; ADR-0018 reading (b), gitea#1008 closed) — shipped two non-Markdown paths under `crates/adapter-linux/src/` after the r45 pin at `93a9aa4`; the r46 sitting (VMID 9475, 2026-08-19 UTC) on its merge commit `199bb06` discharged it, named in the PR body before merge, and WP-020 is re-pinned at `199bb06`. No invocation was void; the guest was created and settled before the PR merged and provisioned on its merge commit, and never rebooted. Nothing that executes changed; the record carves out no path class. Before it: WP-L100 PR #485 — increment 4b's third slice of the Linux read-only inventory adapter: each plain whole device's held standing from sysfs `holders/`, keyed by the holder's own uuid and reported as a state-layer observation, and the cached signature view reported and consulted by nothing, on DR15 — shipped five non-Markdown paths under `crates/adapter-linux/src/` after the r44 pin at `9c14a88`; the r45 sitting (VMID 9474, 2026-08-19 UTC) on its merge commit `93a9aa4` discharged it, named in the PR body before merge, and WP-020 is re-pinned at `93a9aa4`. No invocation was void; the guest was created and settled before the PR merged and provisioned on its merge commit, and never rebooted. A green run says nothing about the slice's correctness — the adapter is a pure library over an injected read seam, outside the loop chain's reach; that evidence is its suite and its six killed mutants. Before it: WP-L100 PR #478 — increment 4b's second slice of the Linux read-only inventory adapter: mdraid arrays named from sysfs `md/uuid` and LVM logical volumes from `dm/name` under their designator-absent volume-group aggregates, on ADR-0053 (spec 17.4.0) — shipped four non-Markdown paths under `crates/adapter-linux/src/` after the r43 pin at `e9c4afe`; the r44 sitting (VMID 9472, 2026-08-18 UTC) on its merge commit `9c14a88` discharged it, named in the PR body before merge, and WP-020 is re-pinned at `9c14a88`. No invocation was void; the guest was created and settled before the PR merged and provisioned on its merge commit, and never rebooted. A green run says nothing about the slice's correctness — the adapter is a pure library over an injected read seam, outside the loop chain's reach; that evidence is its suite and its five killed mutants. Before it: the #1006 arc — WP-010 PR #472 (slice 3q: a designator-absent aggregate is Indeterminate and not an operand, the closure catching up with ADR-0019) and WP-L100 PR #473 (increment 4b's first slice: mdraid arrays as designator-absent aggregates) — shipped six non-Markdown paths after the r42 pin at `46ecad5`; the r43 sitting (VMID 9470, 2026-08-18 UTC) at the arc's head `e9c4afe` discharged it, once for both merges as both PR bodies named before the first merge, and WP-020 is re-pinned at `e9c4afe`. No invocation was void; the guest was created and settled before the arc merged and provisioned on its head, and was not rebooted during the run (a host power loss followed the run's completion; one boot to retrieve, digest re-verified). A green run says nothing about the arc's correctness — the closure and the adapter are pure code outside the loop chain's reach; that evidence is their suites and their four killed mutants. Before it: WP-L100 PR #469 — increment 4a of the Linux read-only inventory adapter: the procfs mount and swap tables as the contract's third interface and the withdrawal of host-assembled block nodes from the physical-device set, on the DR1—DR10 detection rows — shipped eight non-Markdown paths under `crates/adapter-linux/src/` after the r41 pin at `0378bd5`; the r42 sitting (VMID 9469, 2026-08-18 UTC) on its merge commit `46ecad5` discharged it, named in the PR body before merge, and WP-020 is re-pinned at `46ecad5`. No invocation was void; the guest was created and settled before the PR merged and provisioned on its merge commit, and never rebooted. A green run says nothing about the increment's correctness — the adapter is a pure library over an injected read seam, outside the loop chain's reach; that evidence is its suite and its six killed mutants. Before it: the consequence-text body arc — WP-010 PR #461 (slice 3p: plan body version 5, Section 6's consequence text as a canonical set) and WP-060 PR #462 (increment 12: every planning path stating its sentences into it) — shipped six non-Markdown paths after the r40 pin at `0519285`; the r41 sitting (VMID 9466, 2026-08-18 UTC) at the arc's head `0378bd5` discharged it, once for both merges as both PR bodies named before the first merge, and WP-020 is re-pinned at `0378bd5`. No invocation was void; the guest was created and settled before the arc merged and provisioned on its head, and never rebooted. A green run says nothing about the body change's correctness — a plan body is an encoding over an authenticated snapshot, outside the loop chain's reach; that evidence is its suites, its vectors and its nine killed mutants. Before it: the ADR-0052 arc — WP-010 PR #455 (`names_within` public, the `consumed` doc-comment corrected) and WP-060 PR #457 (increment 11: the move, PART-005's destination vocabulary) — shipped five non-Markdown paths after the r39 pin at `7b68927`; the r40 sitting (VMID 9465, 2026-08-18 UTC) at the arc's head `0519285` discharged it, once for both merges as both PR bodies named before the first merge, and WP-020 is re-pinned at `0519285`. No invocation was void and the guest was never rebooted. The identical value set is what the record predicts and not a foregone conclusion: the arc ships product behaviour into the planner and the domain, and none of it is in the loop chain's reach — which is why a green run says nothing about the move's correctness; that evidence is its own suite and its ten killed mutants. Before it: WP-L100 PR #447 — increment 3a of the Linux read-only inventory adapter: devices addressed under ADR-0019 from ADR-0034's designated sources, and INV-004's presentable derivation — shipped five paths under `crates/adapter-linux/src/` after the r38 pin at `31e58be`; the r39 sitting (VMID 9464, 2026-08-18 UTC) on its merge commit discharged it, and WP-020 is re-pinned at `7b68927`. **It breaks a run of three infrastructure trips** — r36, r37 and r38 could not have moved a measured value even in principle, and this one could, so the identical value set is evidence rather than a foregone conclusion. Two invocations were void and are retained in the record: a relative-path launch that repeated custody run 26 verbatim, and the rollback taken to recover it, which returned the guest on the preinstalled -187 kernel. Before it: WP-000 PR #444 — `cargo xtask ci` refuses a documentation claim that has fallen behind the specification — shipped `tools/xtask/src/main.rs` after the r37 pin at `e96e46a`; the r38 sitting (VMID 9462, 2026-08-18 UTC) on its merge commit discharged it, and WP-020 is re-pinned at `31e58be`. That is the third consecutive infrastructure trip — r36 the workflow, r37 the workflow and the tool together, r38 the tool alone — and none touches the product under test. Before it: WP-000 PR #439 — an arc of two: the Section 11.4 fuzz targets run concurrently, and Tier 1 restores only its dependency build from cache — shipped `.github/workflows/ci.yml` and `tools/xtask/src/main.rs` after the r36 pin at `5cf868d`; the r37 sitting (VMID 9461, 2026-08-18 UTC) at the arc's head discharged it. r36 was the first trip that was neither Rust nor a Rust comment; this arc is the first to carry both kinds at once, and neither half touches the product under test. Before it: WP-000 PR #436 — the pinned supply-chain policy tools cached rather than rebuilt from source on every run — shipped `.github/workflows/ci.yml` after the r35 pin at `2428134`; the r36 sitting (VMID 9460, 2026-08-18 UTC) on its merge commit discharged it. That trip is the first in the record that is neither Rust nor a Rust comment — it changes the instrument that measures, not the product measured — and the exemption was declined on the same ground as the test-only trips before it. Before it: issue #371's structural pin (PR #433 — WP-060, test-only), whose r35 sitting (VMID 9459) discharged its own trip, and before that: issue #319's authorization half (PR #428 — ADR-0051, spec 17.2.0), whose r34 sitting (VMID 9458) discharged its own trip, and before that: issue #365's arc (PRs #424 and #425 — ADR-0050, spec 17.1.0), whose r33 sitting (VMID 9457) at the arc's head discharged it, and before that: ADR-0049's planner coverage (PR #421 — WP-060, test-only), whose r32 sitting (VMID 9456) discharged its own trip, and before that: issue #409's act (PR #419 — ADR-0049, spec 17.0.0), whose r31 sitting (VMID 9455) discharged its own trip, and before that: ADR-0048's planner coverage (PR #416 — WP-060, test-only), whose r30 sitting (VMID 9454) discharged its own trip, and before that issue #392's act (PR #414 — ADR-0048, spec 16.0.0), and before that issue #397's act
...
check is
`git diff --name-only e1f138b HEAD | grep -v '\.md$'`, and it must be run
