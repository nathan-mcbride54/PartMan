...
**No sitting is owed at the time of writing.** Issue #392's act (PR #414 — ADR-0048, spec 16.0.0) shipped Rust after the r28 pin at `63ee769`; the r29 sitting (VMID 9453, 2026-08-16 UTC) on its merge commit discharged it, and WP-020 is re-pinned at `be6c3f0`. Before it: issue #397's act
...
check is
`git diff --name-only be6c3f0 HEAD | grep -v '\.md$'`, and it must be run
