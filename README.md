...
**No sitting is owed at the time of writing.** The r21 obligation the
2026-08-15 handoff recorded — a non-Markdown path
(`crates/domain/src/model/protection_tests.rs`, from PR #372) that landed
after WP-020's r20 pin at `6d4a8fc` — was discharged by the r21 sitting
(VMID 9445, 2026-08-15 UTC), and WP-020 is re-pinned at `b8d6a90`. The
check is `git diff --name-only b8d6a90 HEAD | grep -v '\.md$'`, and it
must be run against `HEAD` rather than against whichever pin a pull request
body cited; any non-Markdown path it lists re-opens the three acceptances.
