...
- WP-035 increment 2 delivers the deny-by-default redaction allowlist and the
  ...
  clean bill. Redaction tests gate the tier: the bundle's key set is pinned
  as literals so widening the allowlist is a visible reviewed edit, no
  output in any mode may carry the host's username, home path, computer
  name, or any environment value six bytes or longer, and the bundle must be
  byte-identical across invocations because it reads nothing. The command
  emits to stdout only; the shipped binary still opens no socket, reads no
